...
elf64_sym const* elf64_dynsym_index::operator[](std::string const& str) const
{
	...
	size_t offs			= str.find('@');
	std::string versionless(str.begin(), offs == std::string::npos ? str.end() : (str.begin() + offs));
	uint32_t hash		= __hash(versionless.c_str());
	...
	{
		if(hash_val == (other_hash & ~1U) && !std::strcmp(versionless.c_str(), strtab[sym->st_name]))
			return sym;
		if(other_hash & 1U)
			break;
	}
