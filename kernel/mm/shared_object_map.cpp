...
const char *shared_object_map::get_path(iterator obj) const { if(auto i = __obj_paths.find(obj); i != __obj_paths.end()) { return i->second.c_str(); } else return ""; }
...
extern "C"
{
    addr_t syscall_dlopen(const char* name, int flags)
    {
        ...
        if(!fs_ptr || !task->local_so_map) return addr_t(uintptr_t(-ENOSYS));
        name = translate_user_pointer(name);
        if(!name) return addr_t(uintptr_t(-EINVAL));
        ...
        if(!n) { return addr_t(uintptr_t(-ENOENT)); }
    ...
    int syscall_dlclose(addr_t handle)
    {
        ...
        if(!is_valid_handle(*so)) { return addr_t(uintptr_t(-EINVAL)); }
    ...
    addr_t syscall_dlsym(addr_t handle, const char* name)
    {
        ...
        if(!task->local_so_map) return addr_t(uintptr_t(-ENOSYS));
        name = translate_user_pointer(name);
        if(handle == addr_t(task)) { if(addr_t result = full_search(task, name)) return result; else return addr_t(uintptr_t(-ENOENT)); }
        if(!name) return addr_t(uintptr_t(-EINVAL));
        if(!handle) { if(addr_t result = global_search(name)) return result; else return addr_t(uintptr_t(-ENOENT)); }
        shared_object_map::iterator so(handle);
        if(!is_valid_handle(*so)) { return addr_t(uintptr_t(-EINVAL)); }
    ...
    int syscall_dlorigin(addr_t handle, const char** path_out, size_t* sz_out)
    {
        ...
        std::string p(task->local_so_map->get_path(so));
        const char* q;
        size_t l;
        if(p.empty())
        {
            std::string r(shared_object_map::get_globals().get_path(so));
            if(r.empty()) return -ENOENT;
            q = r.c_str();
            l = r.size();
        }
        else { q = p.c_str(); l = p.size(); }
        uframe_tag* tag = task->task_struct.frame_ptr;
        if(static_cast<size_t>(tag->sysres_extent - tag->sysres_wm) < l)
        {
            kernel_memory_mgr::get().enter_frame(tag);
            addr_t mapping_target = tag->sysres_extent;
            addr_t allocated = kernel_memory_mgr::get().allocate_user_block(l, mapping_target, 0UL, false, false);
            if(!allocated) { kernel_memory_mgr::get().exit_frame(); return -ENOMEM; }
            tag->kernel_allocated_blocks.push_back(allocated);
            tag->sysres_extent += kernel_memory_mgr::get().page_aligned_region_size(mapping_target, l);
            kernel_memory_mgr::get().exit_frame();
        }
        char* result = tag->sysres_wm;
        tag->sysres_wm += l;
        array_copy(result, q, l);
        *path_out = result;
        if(sz_out) *sz_out = l;
        return 0;
    }
