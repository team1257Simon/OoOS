...
void frame_manager::destroy_frame(uframe_tag& ft) { if(__out_of_range(std::addressof(ft))) throw std::out_of_range{ "invalid frame tag" }; erase(const_iterator{ std::addressof(ft) }); }
...
uframe_tag& frame_manager::create_frame(addr_t start_base, addr_t start_extent)
{
    ...
    uframe_tag& result = emplace_back(pt, start_base, start_extent);
