...
bool ahci_hda::init()
{
   ...
   this->__read_fn = [&](qword start, dword count, uint16_t* data) -> bool {  try{ dr->read_sectors(p, start, count, data); return true; } catch(std::exception const& e) { panic(e.what()); return false; } };
   this->__write_fn = [&](qword start, dword count, const uint16_t* data) -> bool { try{ dr->write_sectors(p, start, count, data); return true; } catch(std::exception const& e) { panic(e.what()); return false; }};
   return true;
}
...
std::streamsize ahci_hda::write(uint64_t start_sector, const char *in, uint32_t count)
{
    ...
    if(!__write_fn(start_sector, count, __write_buffer.beg())) { panic("bad write"); return 0; }
    return result;
}