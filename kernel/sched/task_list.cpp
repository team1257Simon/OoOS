...
task_list::iterator task_list::create_system_task(task_functor task, std::vector<const char*>&& args, size_t stack_size, size_t tls_size, priority_val pv, uint16_t quantum) { return this->emplace(task, std::move(args), heap_allocator::get().allocate_kernel_block(stack_size), static_cast<ptrdiff_t>(stack_size), heap_allocator::get().allocate_kernel_block(tls_size), tls_size, __kernel_frame_tag, this->__mk_pid(), 0L, pv, quantum).first; }
task_list::iterator task_list::create_user_task(task_functor task, std::vector<const char *> &&args, vaddr_t frame_ptr, vaddr_t stack_base, vaddr_t tls_base, int64_t parent_pid, size_t stack_size, size_t tls_size, priority_val pv, uint16_t quantum) { return this->emplace(task, std::move(args), stack_base, static_cast<ptrdiff_t>(stack_size), tls_base, tls_size, frame_ptr, this->__mk_pid(), parent_pid, pv, quantum).first; }
task_list::iterator task_list::create_user_task(elf64_desc const &program_desc, std::vector<const char *> &&args, int64_t parent_pid, priority_val pv, uint16_t quantum) { return create_user_task(vaddr_t{ program_desc.entry }, std::move(args), program_desc.frame_ptr, program_desc.prg_stack, program_desc.prg_tls, parent_pid, program_desc.stack_size, program_desc.tls_size, pv, quantum); }
