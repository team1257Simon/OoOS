...
task_ctx* task_list::create_system_task(task_functor task, std::vector<const char*>&& args, size_t stack_size, size_t tls_size, priority_val pv, uint16_t quantum) { iterator result = emplace(task, std::move(args), kernel_memory_mgr::get().allocate_kernel_block(stack_size), static_cast<ptrdiff_t>(stack_size), kernel_memory_mgr::get().allocate_kernel_block(tls_size), tls_size, __kernel_frame_tag, __mk_pid(), 0L, pv, quantum).first; result->init_task_state(); return result.base(); }
task_ctx* task_list::create_user_task(task_functor task, std::vector<const char*>&& args, addr_t frame_ptr, addr_t stack_base, addr_t tls_base, int64_t parent_pid, size_t stack_size, size_t tls_size, priority_val pv, uint16_t quantum) { iterator result = emplace(task, std::move(args), stack_base, static_cast<ptrdiff_t>(stack_size), tls_base, tls_size, frame_ptr, __mk_pid(), parent_pid, pv, quantum).first; result->init_task_state(); return result.base(); }
...
bool task_list::destroy_task(uint64_t pid) { iterator i = find(pid); if(i == end()) return false; scheduler::get().unregister_task_tree(i->task_struct.self); erase(i); return true; }
