...
task_ctx* task_list::create_user_task(elf64_program_descriptor const& program_desc, std::vector<const char*>&& args, int64_t parent_pid, priority_val pv, uint16_t quantum) { iterator result = emplace(program_desc, std::move(args), __mk_pid(), parent_pid, pv, quantum).first; result->init_task_state(); return result.base(); }
