...
task_ctx* task_list::create_system_task(task_functor task, std::vector<const char*>&& args, size_t stack_size, size_t tls_size, priority_val pv, uint16_t quantum) { iterator result = this->emplace(task, std::move(args), kernel_memory_mgr::get().allocate_kernel_block(stack_size), static_cast<ptrdiff_t>(stack_size), kernel_memory_mgr::get().allocate_kernel_block(tls_size), tls_size, __kernel_frame_tag, this->__mk_pid(), 0L, pv, quantum).first; result->init_task_state(); return result.base(); }
