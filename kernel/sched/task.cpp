...
void task_ctx::__init_task_state(task_functor task, vaddr_t stack_base, ptrdiff_t stack_size, vaddr_t tls_base, vaddr_t frame_ptr)
{
    ...
    if(is_system())
    {
        ...
        task_struct.saved_regs.ds = task_struct.saved_regs.es = task_struct.saved_regs.fs = task_struct.saved_regs.gs = 0x10;
    }
    else
    {
        task_struct.saved_regs.cs = 0x20;
        task_struct.saved_regs.ds = task_struct.saved_regs.es = task_struct.saved_regs.fs = task_struct.saved_regs.gs = 0x18;
    }
}
task_ctx::task_ctx(task_functor task, std::vector<const char*>&& args, vaddr_t stack_base, ptrdiff_t stack_size, vaddr_t tls_base, size_t tls_len, vaddr_t frame_ptr, uint64_t pid, int64_t parent_pid, priority_val prio, uint16_t quantum) : task_struct { &task_struct, frame_ptr, regstate_t{}, quantum, 0U, tcb_t { { false, false, false, false }, 0U, prio, 0U, 0U, parent_pid, pid, 0UL, 0UL }, fx_state{}, 0UL, nullptr, tls_base, nullptr }, arg_vec{ std::move(args) }, allocated_stack{ stack_base }, stack_allocated_size{ static_cast<size_t>(stack_size) }, tls{ tls_base }, tls_size{ tls_len } { __init_task_state(task, stack_base, stack_size, tls_base, frame_ptr); }
