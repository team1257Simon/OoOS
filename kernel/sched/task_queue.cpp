...
bool task_wait_queue::interrupt_wait(const_iterator where) { if(__unlikely(!(where < end() && (*where)->task_ctl.can_interrupt))) return false; if(const_iterator subs = where + 1; subs < end()) (*subs)->task_ctl.wait_ticks_delta += (*where)->task_ctl.wait_ticks_delta; (*where)->task_ctl.block = false; return erase(where) != 0; }
...
__isrcall task_pl_queue& prio_level_task_queues::operator[](priority_val pv) noexcept { return __base::operator[](__idx_by_prio(pv)); }
__isrcall task_pl_queue const& prio_level_task_queues::operator[](priority_val pv) const noexcept { return __base::operator[](__idx_by_prio(pv)); }