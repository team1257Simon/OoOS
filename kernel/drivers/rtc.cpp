...
void rtc::init_instance(uint8_t century_register) noexcept { __instance.__century_register = century_register; __instance.__is_12h = !(read_rtc_register<0x0BUC>().b1); __instance.__is_bcd = !(read_rtc_register<0x0BUC>().b2); interrupt_table::add_irq_handler(0UC, std::move(std::bind(&rtc::rtc_time_update, std::addressof(__instance)))); }
