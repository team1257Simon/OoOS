...
pci_config_table* find_pci_config(xsdt_t* xsdt) { if(xsdt) return addr_t{ find_system_table(xsdt, "MCFG") }; return nullptr; }
...
addr_t compute_base_address(uint32_t bar_registers[], uint8_t i) { return addr_t{ uintptr_t((bar_registers[i] & 0x00000001) ? uintptr_t(bar_registers[i] & 0xFFFFFFFC) : uint64_t((bar_registers[i] & 0xFFFFFFF0uL) | (bar_registers[i] & 0x00000004 ? (uint64_t(bar_registers[i+1]) << 32) : 0))) }; }
addr_t compute_base(uint32_t bar) { return addr_t{ ((bar & 0x00000001) ? uintptr_t(bar & 0xFFFFFFFC) : uintptr_t(bar & 0xFFFFFFF0)) }; }
