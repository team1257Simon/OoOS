...
pci_config_space* get_device(pci_config_table* tb, uint8_t bus, uint8_t slot, uint8_t func)
{
	for(uint8_t i = 0UC; i < static_cast<uint8_t>((tb->hdr.length - sizeof(tb->hdr) - 8UC) / 16UC); i++)
		if(tb->addr_allocations[i].start_bus <= bus && tb->addr_allocations[i].end_bus >= bus)
			return check_valid(tb->addr_allocations[i].config_start + ((bus - tb->addr_allocations[i].start_bus) * 256 + slot * 8 + func));
	return nullptr;
}
void pci_device_list::add_all(pci_config_table* tb)
{
	for(int i = 0; i < 256; i++)
	{
		for(int j = 0; j < 32; j++)
		{
			pci_config_space* p = __add(get_device(tb, i, j, 0));
			if(p && p->header_type & BIT(7))
				for(int k = 1; k < 8; k++)
					__add(get_device(tb, i, j, k));
		}
	}
}
pci_config_space* pci_device_list::find(uint8_t device_class, uint8_t subclass)
{
...
bool pci_device_list::init_instance()
{
	pci_config_table* tb = find_pci_config();
	if(__unlikely(!tb)) return false;
