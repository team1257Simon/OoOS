...
addr_t compute_base_address(uint32_t bar_registers[], uint8_t i) { return addr_t{ static_cast<uintptr_t>((bar_registers[i] & 0x00000001UL) ? static_cast<uint64_t>(bar_registers[i] & 0xFFFFFFFCUL) : static_cast<uint64_t>((bar_registers[i] & 0xFFFFFFF0UL) | (bar_registers[i] & 0x00000004UL ? (static_cast<uint64_t>(bar_registers[i + 1]) << 32) : 0UL))) }; }
