...
namespace ooos
{
	...
	attribute(nointerrupts) bool ps2_init(ps2_controller& ps2) noexcept
	{
		try
		{
			...
			ps2 << PSC_MEM_READ;
			bool read 		= false;
			for(time_t spin = 0UL; spin < ps2.io_timeout && !read; spin++, read = ps2 >> cfg);
			if(!read) throw std::runtime_error("[PS2] controller timeout");
			...
			read 			= false;
			uint8_t response;
			for(time_t spin = 0UL; spin < ps2.io_timeout && !read; spin++, read = ps2 >> response);
			if(!read) throw std::runtime_error("[PS2] controller timeout");
