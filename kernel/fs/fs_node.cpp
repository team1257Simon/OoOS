...
void inode_base::unregister_reference(tnode *ref) { if(refs.erase(ref)) ref->invlnode(); }
void inode_base::prune_refs() { for(tnode* ref : refs) ref->invlnode(); refs.clear(); }
bool inode_base::has_refs() const noexcept { return refs.size() != 0; }
size_t inode_base::num_refs() const noexcept { return refs.size(); }
...
bool folder_inode_base::is_empty() const noexcept { return this->size() == 0; }
