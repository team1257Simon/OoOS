...
tnode::tnode(inode_base* node, std::string const& name) : __my_node{ node }, __my_name{ name } {}
tnode::tnode(inode_base *node, const char *name) : __my_node{node}, __my_name{name} {}
...
inode_base *tnode::operator->() noexcept { return __my_node; }
inode_base const *tnode::operator->() const noexcept { return __my_node; }
bool tnode::if_file(std::function<bool(file_inode_base &)> const &action) { return __my_node->is_file() && action(dynamic_cast<file_inode_base&>(*__my_node)); }
bool tnode::if_folder(std::function<bool(folder_inode_base &)> const &action) { return __my_node->is_folder() && action(dynamic_cast<folder_inode_base&>(*__my_node)); }
bool tnode::if_file(std::function<bool(file_inode_base const &)> const &action) const { return  __my_node->is_file() && action(dynamic_cast<file_inode_base const&>(*__my_node)); }
bool tnode::if_folder(std::function<bool(folder_inode_base const &)> const &action) const { return __my_node->is_folder() && action(dynamic_cast<folder_inode_base const&>(*__my_node)); }
...
bool tnode::is_file() const { return __my_node->is_file(); }
bool tnode::is_folder() const { return __my_node->is_folder(); }
