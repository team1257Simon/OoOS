...
bool ext_node_extent_tree::parse_legacy()
{
    ...
    if(!tracked_node->parent_fs->read_unbuffered(*single_ptr_block)) { panic("read on single pointer block failed"); return false; }
    ...
    try 
    {
        ...
        if(!tracked_node->parent_fs->read_unbuffered(*di_pointer_block)) { panic("read on double pointer block failed"); return false; }
        ...
        exnode = base_extent_level.insert(std::move(std::make_pair(cur_file_block, cached_node_pos(base)))).first;
        if(!tracked_node->parent_fs->read_unbuffered(*tri_pointer_block)) { panic("read on triple pointer block failed"); return false; }
...
bool ext_node_extent_tree::parse_ext4()
{
    ...
    for(size_t i = 0; i < num; i++)
    {
        if(base_depth)
        {
            ...
            if(!tracked_node->parent_fs->read_unbuffered(*blk)) { panic("metadata block read failed"); return false; }
...
size_t cached_extent_node::nl_recurse_legacy(ext_node_extent_tree* parent, uint64_t start_file_block)
{
    ...
    for(size_t i = 0; i < total_dwords; i++, parent->total_extent++)
    {
        ...
        if(blk->data_buffer && !tracked_node->parent_fs->read_unbuffered(*blk)) { throw std::runtime_error{ "failed to read disk block" }; }
