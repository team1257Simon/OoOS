...
bool ext_node_extent_tree::parse_legacy()
{
    ...
    std::map<uint64_t, cached_extent_node*>::iterator exnode = base_extent_level.insert(std::make_pair(cur_file_block, std::addressof(tracked_extents.emplace_back(single_ptr_block, tracked_node, uint16_t(1))))).first;
    try 
    {
...
bool ext_node_extent_tree::parse_ext4()
{
    ...
    for(size_t i = 0; i < num; i++)
    {
        if(uint16_t(h->depth))
        {
            ...
            std::map<uint64_t, cached_extent_node*>::iterator exnode = base_extent_level.insert(std::make_pair(cur_file_block, std::addressof(tracked_extents.emplace_back(blk, tracked_node, uint16_t(h->depth))))).first;
        ...
        else 
        {
            ...
            base_extent_level.insert_or_assign(cur_file_block, std::addressof(tracked_extents.emplace_back(blk, tracked_node, uint16_t(0))));
            total_extent += ext_sz;
        }
    }
    return true;
...
bool cached_extent_node::nl_recurse_ext4(ext_node_extent_tree* parent, uint64_t start_file_block)
{
    ...
    for(size_t i = 0; i < n_nodes; i++)
    {
        if(uint16_t(h->depth))
        {
            ...
            disk_block* blk = std::addressof(tracked_node->cached_metadata.emplace_back(blknum, tracked_node->parent_fs->allocate_block_buffer(), false, 1U));
            if(!tracked_node->parent_fs->read_from_disk(*blk)) { panic("metadata block read failed"); return false; }
            std::map<uint64_t, cached_extent_node*>::iterator exnode = next_level_extents.insert(std::make_pair(cur_file_block, std::addressof(parent->tracked_extents.emplace_back(blk, tracked_node, uint16_t(h->depth))))).first;
        ...
        else
        {
            ...
            uint64_t blknum = qword(nodes[i].leaf.extent_start_lo, uint32_t(nodes[i].leaf.extent_start_hi));
            disk_block* blk = std::addressof(tracked_node->block_data.emplace_back(blknum, nullptr, false, ext_sz));
            next_level_extents.insert_or_assign(cur_file_block, std::addressof(parent->tracked_extents.emplace_back(blk, tracked_node, uint16_t(0))));
