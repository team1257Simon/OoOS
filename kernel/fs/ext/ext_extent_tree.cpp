...
bool ext_node_extent_tree::parse_legacy()
{
    ...
    auto exnode = base_extent_level.insert(std::make_pair(cur_file_block, cached_node_pos(base))).first;
    try 
    {
...
bool ext_node_extent_tree::parse_ext4()
{
    ...
    for(size_t i = 0; i < num; i++)
    {
        if(uint16_t(h->depth))
        {
            ...
            cached_extent_node* base = std::addressof(tracked_extents.emplace_back(blk, tracked_node, uint16_t(h->depth)));
            auto exnode = base_extent_level.insert(std::make_pair(cur_file_block, cached_node_pos(base))).first;
        ...
        else 
        {
            ...
            base_extent_level.insert_or_assign(cur_file_block, cached_node_pos(tracked_extents.emplace_back(blk, tracked_node, uint16_t(0))));
            total_extent += ext_sz;
        }
    }
    has_init = true;
...
size_t cached_extent_node::nl_recurse_legacy(ext_node_extent_tree* parent, uint64_t start_file_block)
{
    ...
    for(size_t i = 0; i < total_dwords; i++, parent->total_extent++)
    {
        ...
        disk_block* blk = depth == 1 ? std::addressof(tracked_node->block_data.emplace_back(blk_ptrs[i], nullptr, false, 1U)) : std::addressof(tracked_node->cached_metadata.emplace_back(blk_ptrs[i], tracked_node->parent_fs->allocate_block_buffer(), false, 1U));
        if(blk->data_buffer && !tracked_node->parent_fs->read_from_disk(*blk)) { throw std::runtime_error{ "failed to read disk block" }; }
        cached_extent_node* base = std::addressof(parent->tracked_extents.emplace_back(blk, tracked_node, uint16_t(depth - 1)));
...
bool cached_extent_node::nl_recurse_ext4(ext_node_extent_tree* parent, uint64_t start_file_block)
{
    ...
    for(size_t i = 0; i < n_nodes; i++)
    {
        if(uint16_t(h->depth))
        {
            ...
            disk_block* blk = std::addressof(tracked_node->cached_metadata.emplace_back(blknum, tracked_node->parent_fs->allocate_block_buffer(), false, 1U));
            if(!tracked_node->parent_fs->read_from_disk(*blk)) { panic("metadata block read failed"); return false; }
            cached_extent_node* base = std::addressof(parent->tracked_extents.emplace_back(blk, tracked_node, uint16_t(h->depth)));
            auto exnode = next_level_extents.insert(std::make_pair(cur_file_block, parent->cached_node_pos(base))).first;
        ...
        else
        {
            ...
            uint64_t blknum = qword(nodes[i].leaf.extent_start_lo, uint32_t(nodes[i].leaf.extent_start_hi));
            disk_block* blk = std::addressof(tracked_node->block_data.emplace_back(blknum, nullptr, false, ext_sz));
            next_level_extents.insert_or_assign(cur_file_block, parent->cached_node_pos(parent->tracked_extents.emplace_back(blk, tracked_node, uint16_t(0))));
...
uint64_t cached_extent_node::get_disk_blocknum_recurse(ext_node_extent_tree* parent, uint64_t file_block)
{
    ...
    cached_extent_node* base = parent->get_cached(i->second);
    if(!base) return 0;
    if(base->depth > 1) return base->get_disk_blocknum_recurse(parent, file_block);
    disk_block* b = base->block();
    if(file_block < b->chain_len + i->first) return b->block_number + (file_block % i->first);
    return 0;
}
uint64_t ext_node_extent_tree::get_disk_blocknum(uint64_t file_block)
{
    ...
    cached_extent_node* base = get_cached(i->second);
    if(!base) return 0;
    if(base->depth > 1) return base->get_disk_blocknum_recurse(this, file_block);
    disk_block* b = base->block();
    if(file_block < b->chain_len + i->first) return b->block_number + (file_block % i->first);
    return 0;
}
disk_block *cached_extent_node::get_block_recurse(ext_node_extent_tree* parent, uint64_t file_block)
{
    ...
    if(base->depth > 1) return base->get_block_recurse(parent, file_block);
    disk_block* b = base->block();
    if(file_block < b->chain_len + i->first) return b;
    return nullptr;
}
disk_block *ext_node_extent_tree::get_extent_block(uint64_t file_block)
{
    ...
    if(base->depth > 1) return base->get_block_recurse(this, file_block);
    disk_block* b = base->block();
    if(file_block < b->chain_len + i->first) return b;
    return nullptr;
}
bool ext_node_extent_tree::push_extent_legacy(disk_block *blk)
{
    ...
    auto last = base_extent_level.max();
    if(last == base_extent_level.end())
    {
        ...
        { 
            ...
            base_extent_level.insert(std::make_pair(this->total_extent, cached_node_pos(base))).first;
        ...
        if(!base->push_extent_recurse_legacy(this, blk))
        {
            if(base->depth < 3)
            {
                ...
                { 
                    ...
                    base_extent_level.insert(std::make_pair(total_extent, cached_node_pos(base))).first;
...
bool cached_extent_node::push_extent_recurse_legacy(ext_node_extent_tree *parent, disk_block *blk)
{
    ...
    if(depth < 2)
    {
        uint32_t* bptrs = reinterpret_cast<uint32_t*>(this->block()->data_buffer);
        for(size_t i = 0; i < ndwords; i++)
        {
            if(!bptrs[i]) 
            { 
                bptrs[i] = static_cast<uint32_t>(blk->block_number); 
                next_level_extents.insert_or_assign(parent->total_extent++, parent->cached_node_pos(parent->tracked_extents.emplace_back(blk, tracked_node, uint16_t(0)))); 
                block()->dirty = true;
                return true;
            }
        }
    ...
    if(last == next_level_extents.end() || !parent->get_cached(last->second)->push_extent_recurse_legacy(parent, blk))
    {
        ...
        if(!nl_blk) throw std::runtime_error{ "failed to allocate block" };
...
{
    ext_extent_header* hdr = reinterpret_cast<ext_extent_header*>(block()->data_buffer);
    if(hdr->magic != ext_extent_magic) { throw std::runtime_error{ "invalid header" }; }
    if(depth < 2)
    {
        if(hdr->entries < hdr->max_entries)
        {
            ext_extent_node* node = reinterpret_cast<ext_extent_node*>(block()->data_buffer + sizeof(ext_extent_header)) + hdr->entries;
