...
void ext_block_group::compute_checksums(size_t group_num)
{
    ...
    if(parent_fs->sb->read_only_optional_features & metadata_csum)
    {
        ...
        barrier();
        dword cs_full = parent_fs->uuid_csum;
...
{
    sb->uuid_checksum = uuid_csum;
    sb->checksum = 0;
    uint32_t csum = crc32c(*sb);
    sb->checksum = csum;
