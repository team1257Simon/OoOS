...
int fat32_filebuf::__ddwrite() { size_t n = 0; for(size_t i = 0; i < __my_clusters.size() && n < this->__size(); i++, n += physical_block_size) { if(!ahci_hda::write(__parent->cl_to_s(__my_clusters[i]), this->__get_ptr(n), 1)) { return -1; } } return 0; }
std::streamsize fat32_filebuf::__ddread(std::streamsize n)
{
    ...
    for(size_t i = 0; i < s && __next_cluster_idx < __my_clusters.size(); i++, ++__next_cluster_idx) { if(size_t r = ahci_hda::read(this->__beg() + k, __parent->cl_to_s(__my_clusters[__next_cluster_idx]), 1)) { k += r; } else break; }
