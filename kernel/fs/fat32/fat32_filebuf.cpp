...
int fat32_filebuf::write_dev() { size_t bs = __parent->parent_fs->block_size(); size_t n = 0UZ; for(size_t i = 0UZ; i < __my_clusters.size() && n < __size(); i++, n += bs) { if(!__parent->parent_fs->write_clusters(__my_clusters[i], this->__get_ptr(n))) { return -1; } } return 0; }
...
std::streamsize fat32_filebuf::read_dev(std::streamsize n)
{
	...
	for(size_t i = 0UZ; i < s && __next_cluster_idx < __my_clusters.size(); i++, ++__next_cluster_idx) { if(__parent->parent_fs->read_clusters(__get_ptr(k), __my_clusters[i])) { k += bs; } else break; }
...
std::streamsize fat32_filebuf::on_overflow(std::streamsize n)
{
	...
	size_t s	= div_round_up(n, bs);
	size_t k	= 0UZ;
	for(size_t i = 0UZ; i < s; i++, k += bs) { if(uint32_t cl = __parent->claim_next(__my_clusters.back())) { __my_clusters.push_back(cl); } else break; }
	if(!__grow_buffer_exact(k)) return 0UZ;
	return k;
}
