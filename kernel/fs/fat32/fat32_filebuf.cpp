...
int fat32_filebuf::__ddwrite() { size_t n = 0; for(size_t i = 0; i < __my_clusters.size() && n < this->__size(); i++, n += physical_block_size) { if(!ahci_hda::write(__cluster_to_sector_fn(__my_clusters[i]), this->__get_ptr(n), 1)) { return -1; } } return 0; }
