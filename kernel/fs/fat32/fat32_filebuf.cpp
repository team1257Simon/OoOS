...
std::streamsize fat32_filebuf::read_dev(std::streamsize n)
{
    ...
    for(size_t i = 0; i < s && __next_cluster_idx < __my_clusters.size(); i++, ++__next_cluster_idx) { if(__parent->parent_fs->read_clusters(__get_ptr(k), __my_clusters[i])) { k += physical_block_size * __parent->parent_fs->__sectors_per_cluster; } else break; }
