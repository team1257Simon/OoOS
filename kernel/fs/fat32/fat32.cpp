...
fat32::fat32(fat32_bootsect const &bootsect, uint64_t start_sector) : __sectors_per_cluster{ bootsect.sectors_per_cluster }, __sector_base{ start_sector + bootsect.num_reserved_sectors + (bootsect.num_fats * bootsect.fat_size) }, __dev_serial{ bootsect.volume_serial }, __the_table{ bootsect.num_fats * bootsect.fat_size, bootsect.bytes_per_sector, start_sector + bootsect.num_reserved_sectors }, __root_directory{ this, "", bootsect.root_cluster_num } {}
...
file_inode* fat32::mkfilenode(folder_inode* parent, std::string const& name)
{
    ...
    std::set<fat32_file_inode>::iterator i = __opened_files.emplace(this, name, std::addressof(avail->regular_entry)).first;
...
folder_inode* fat32::mkdirnode(folder_inode* parent, std::string const& name)
{
    ...
    avail->regular_entry.first_cluster_hi = cl.hi;
    avail->regular_entry.first_cluster_lo = cl.lo;
    std::set<fat32_folder_inode>::iterator i = __opened_folders.emplace(this, name, std::addressof(avail->regular_entry)).first;
    add_start_cluster_ref(i->start_cluster());
    return std::addressof(*i);
}
