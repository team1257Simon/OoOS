...
void ramfs::dldirnode(directory_node* dd) { if(!dd->is_empty()) { throw std::logic_error{ std::string{ "cannot delete directory " } + dd->name() + " because it is not empty" }; } dd->prune_refs(); __directory_nodes.erase(*dd); this->syncdirs(); }
