...
void ramfs::dlfilenode(file_inode* fd) { fd->prune_refs(); __file_nodes.erase(*fd); this->syncdirs(); }
void ramfs::dldirnode(folder_inode* dd) { if(!dd->is_empty()) { throw std::logic_error{ std::string{ "cannot delete folder " } + dd->name() + " because it is not empty" }; } dd->prune_refs(); __folder_nodes.erase(*dd); this->syncdirs(); }
