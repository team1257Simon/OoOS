...
bool ramfs_directory_inode::fsync() { for(tnode_dir::iterator first = __my_dir.begin(), last = __my_dir.end(); first != last; ) { if(!first->operator bool()) first = __my_dir.erase(first); else ++first; } return true; }
...
uint64_t ramfs_directory_inode::num_subdirs() const noexcept { return __my_subdir_cnt; }
uint64_t ramfs_directory_inode::num_files() const noexcept { return __my_file_cnt; }
bool ramfs_directory_inode::unlink(std::string const &what) { bool result = __my_dir.erase(what) != 0; if(result) syscall_time(&this->modif_time); return result; }
tnode* ramfs_directory_inode::add(fs_node* n) { std::pair<tnode_dir::iterator, bool> result = __my_dir.emplace(n, n->name()); if(result.second) syscall_time(&this->modif_time); return result.first.base(); }
bool ramfs_directory_inode::link(tnode* original, std::string const& alias) { bool result = __my_dir.emplace(mklink(original, alias)).second; if(result) syscall_time(&this->modif_time); return result; }
