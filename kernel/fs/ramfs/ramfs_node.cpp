...
#include "rtc.h"
ramfs_folder_inode::ramfs_folder_inode(std::string const& name) : folder_inode{ name, reinterpret_cast<uint64_t>(this) } {}
...
ramfs_file_inode::ramfs_file_inode(std::string const& name, int fd) : file_inode{ name, fd, reinterpret_cast<uint64_t>(this) } {}
