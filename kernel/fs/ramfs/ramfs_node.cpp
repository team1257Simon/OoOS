...
bool ramfs_directory_vnode::truncate()
{
...
bool ramfs_directory_vnode::unlink(std::string const& what)
{
	bool result = (directory_tnodes.erase(what) != 0UZ);
	if(result) sys_time(std::addressof(modif_time));
	return result;
}
tnode* ramfs_directory_vnode::add(vnode* n)
{
	std::pair<tnode_dir::iterator, bool> result = directory_tnodes.emplace(n, n->name());
	if(result.second) sys_time(std::addressof(modif_time));
	return result.first.base();
}
bool ramfs_directory_vnode::link(tnode* original, std::string const& alias)
{
	bool result = directory_tnodes.emplace(mklink(original, alias)).second;
	if(result) sys_time(std::addressof(modif_time));
	return result;
}
ramfs_file_vnode::size_type ramfs_file_vnode::write(const_pointer src, size_type n)
{
	size_t result = sputn(src, n);
	if(result) sys_time(std::addressof(modif_time));
	return result;
}
bool ramfs_file_vnode::grow(size_t added)
{
