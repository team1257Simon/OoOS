...
bool filesystem::xunlink(directory_node* parent, std::string const& what, bool ignore_nonexistent, bool dir_recurse)
{
    ...
    if(!(*node)->has_refs()) { if(node->is_file()){ if(node->as_file()->is_device()) { this->dldevnode(dynamic_cast<device_node*>(node->as_file())); } else this->dlfilenode(node->as_file()); } if(node->is_folder()) this->dldirnode(node->as_folder()); }
    return true;
}
...
file_node* filesystem::open_file(std::string const& path, std::ios_base::openmode mode)
{
    ...
    if(node && node->is_folder()) throw std::logic_error{ "path " + path + " exists and is a directory" };
    if(!node) 
    {
