...
filesystem::target_pair filesystem::get_parent(std::string const& path, bool create)
{
    ...
    for(size_t i = 0; i < pathspec.size() - 1; i++)
    {
        ...
        if(!cur)
        {
            ...
            else{ throw std::logic_error{ "folder " + pathspec[i] + " does not exist (use get_folder(\".../" + pathspec[i] + "\", true) to create it)" };}
        }
...
folder_inode* filesystem::get_folder(std::string const& path, bool create)
{
    ...
    if(!node)
    {
        ...
        else throw std::logic_error{ "path " + path + " does not exist (use get_folder(\"" + path + "\", true) to create it)" };
    }
...
static inline void __st_intl(inode* n, filesystem* fsptr, stat* st) { new (translate_user_pointer(st)) stat{ fsptr->get_dev_id(), n->cid(), n->mode, n->num_refs(), 0, 0, n->is_device() ? 1U : 0, static_cast<long>(n->size()), timestamp_to_timespec(n->create_time), timestamp_to_timespec(n->modif_time), timestamp_to_timespec(n->modif_time), 512, n->size() / 512 };  }
extern "C"
{
