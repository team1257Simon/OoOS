...
protocol_ethernet::protocol_ethernet(net_device& dev) : abstract_protocol_handler(nullptr, this), ip_resolver(dev.get_ip_resolver()), transmit_fn(std::bind(&net_device::transmit, std::addressof(dev), std::placeholders::_1)), handlers(64UZ), mac_addr(dev.get_mac_addr()) {}
