...
namespace ooos
{
	...
	struct __pack xhci_trb_type_and_flags
	{
		...
		bool ios_or_event_data			: 1;	// for transfer events, event data; for transfers, interrupt-on-short-packet; otherwise, zero
	...
	struct xhci_hc_mem
	{
		struct __pack
		{
			...
			bool								: 1;
			bool cem_enable						: 1;	// enable max exit latency too large error
	...
	typedef xhci_transfer_trb<setup_trb_dw3hi, usb_control_transfer_setup> xhci_setup_trb;
