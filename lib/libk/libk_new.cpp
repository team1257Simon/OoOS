...
namespace std
{
    [[gnu::externally_visible]] void* __detail::__aligned_reallocate(void* ptr, size_t n, size_t align) { if(void* result = __kernel_frame_tag->reallocate(ptr, n, align)) return result; else if(std::new_handler h = std::get_new_handler()) h(); else throw std::bad_alloc{}; return __kernel_frame_tag->reallocate(ptr, n, align); }
