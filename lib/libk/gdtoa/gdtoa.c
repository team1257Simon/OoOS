...
static big_int* bitstob(uilong* bits, int nbits, int* bbits)
{
    ...
    be = bits + ((nbits - 1) >> 5);
    x = x0 = b->x;
    do { *x++ = *bits & 0xFFFFFFFF; } while (++bits <= be);
    i = x - x0;
    while (!x0[--i])    if (!i) { b->wds = 0; *bbits = 0; goto ret; }
    b->wds = i + 1;
...
char* __gdtoa(fpi* fpi, int be, uilong* bits, int* kindp, int mode, int ndigits, int* decpt, char** rve)
{
    ...
    if (k >= 0 && k <= 22) { if ((&d)->d < __tens_d2a[k]) k--; k_check = 0; }
    j = bbits - i - 1;
    if (j >= 0) { b2 = 0; s2 = j; } 
    else { b2 = -j; s2 = 0; }
    if (k >= 0) { b5 = 0; s5 = k; s2 += k; } 
    else { b2 -= k; b5 = -k; s5 = 0; }
    if (mode < 0 || mode > 9) mode = 0;
    try_quick = 1;
    if (mode > 5) { mode -= 4; try_quick = 0; } 
    else if (i >= -4 - (-1022) || i < (-1022)) try_quick = 0;
    leftright = 1;
    ilim = ilim1 = -1;
    switch (mode) 
    {
    ...
    if ((rdir = fpi->rounding - 1) != 0) { if (rdir < 0) rdir = 2; if (kind & strtog_neg) rdir = 3 - rdir; }
    if (ilim >= 0 && ilim <= 14 && try_quick && !rdir && k == 0) 
    {
        ...
        if (leftright) 
        {
            ...
            for (i = 0; ; )
            {
                ...
                (&d)->d -= u_l * ds;
                *s++ = '0' + (int)u_l;
                if ((&d)->d < (&eps)->d) { if ((&d)->d) inex = strtog_inexlo; goto ret1; }
        ...
        else 
        {
            ...
            for (i = 1;; i++, (&d)->d *= 10.) 
            {
                ...
                *s++ = '0' + (int)u_l;
                ...
                {
                    ds *= 0.5;
                    if ((&d)->d > ds + (&eps)->d) goto bump_up;
                    else if ((&d)->d < ds - (&eps)->d) { if ((&d)->d) inex = strtog_inexlo; goto clear_trailing0; }
                    break;
                }
            }
        }
    ...
    {
        i = nbits - bbits;
        if (be - i++ < fpi->emin && mode != 3 && mode != 5) { i = be - fpi->emin + 1; if (mode >= 2 && ilim > 0 && ilim < i) goto small_ilim; } 
        else if (mode >= 2) 
        {
        small_ilim:
            j = ilim - 1;
    ...
    if (b5 > 0) 
    {
        if (leftright) 
        {
            ...
            if ((j = b5 - m5) != 0) { b = __pow5mult_d2a(b, j); if (b == NULL) return (NULL); }
        }
        else { b = __pow5mult_d2a(b, b5);    if (b == NULL) return (NULL); }
    }
    S = __i2b_d2a(1);
    if (S == NULL) return (NULL);
    if (s5 > 0) { S = __pow5mult_d2a(S, s5); if (S == NULL) return (NULL); }
    spec_case = 0;
    if (mode < 2) 
    {
    ...
    m2 += i;
    if ((b2 += i) > 0) { b = __lshift_d2a(b, b2); if (b == NULL) return (NULL); }
    if ((s2 += i) > 0) { S = __lshift_d2a(S, s2); if (S == NULL) return (NULL); }
    if (k_check) 
    {
        if (__cmp_d2a(b, S) < 0) 
        {
            ...
            if (b == NULL) return (NULL);
            if (leftright) { mhi = __multadd_d2a(mhi, 10, 0); if (mhi == NULL) return (NULL); }
            ilim = ilim1;
        }
    }
    if (ilim <= 0 && mode > 2) 
    ...
    if (leftright) 
    {
        if (m2 > 0) { mhi = __lshift_d2a(mhi, m2); if (mhi == NULL) return (NULL); }
        mlo = mhi;
        if (spec_case) 
        {
        ...
        for (i = 1;; i++) 
        {
            ...
            if (j1 == 0 && !mode && !(bits[0] & 1) && !rdir) 
            {
                ...
                if (j <= 0) { if (b->wds > 1 || b->x[0]) inex = strtog_inexlo; } 
                else { dig++; inex = strtog_inexhi; }
                *s++ = dig;
                goto ret;
            }
            if (j < 0 || (j == 0 && !mode && !(bits[0] & 1))) 
            {
                ...
                {
                    if (rdir == 2) { inex = strtog_inexlo; goto accept; }
            ...
            *s++ = dig;
            ...
            b = __multadd_d2a(b, 10, 0);
            if (b == NULL) return (NULL);
            if (mlo == mhi) { mlo = mhi = __multadd_d2a(mhi, 10, 0); if (mlo == NULL) return (NULL); } 
            else 
            {
    ...
    if (rdir) { if (rdir == 2 || (b->wds <= 1 && !b->x[0])) goto chopzeros; goto roundoff; }
    b = __lshift_d2a(b, 1);
    if (b == NULL) return (NULL);
    j = __cmp_d2a(b, S);
    if (j > 0 || (j == 0 && dig & 1)) 
...
ret:
    __bfree_d2a(S);
    if (mhi) { if (mlo && mlo != mhi) __bfree_d2a(mlo); __bfree_d2a(mhi); }
ret1:
    __bfree_d2a(b);
    *s = 0;
    *decpt = k + 1;
