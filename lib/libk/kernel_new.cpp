...
[[nodiscard]] void* operator new(std::size_t count) throw() { if(void* ptr = __kernel_frame_tag->allocate(count)) return ptr; else if(std::new_handler h = std::get_new_handler()) h(); else throw std::bad_alloc{}; return __kernel_frame_tag->allocate(count); }
[[nodiscard]] void* operator new[](std::size_t count) throw() { if(void* ptr = __kernel_frame_tag->allocate(count)) return ptr; else if(std::new_handler h = std::get_new_handler()) h();  else throw std::bad_alloc{}; return __kernel_frame_tag->allocate(count); }
[[nodiscard]] void* operator new(std::size_t count, std::align_val_t al) throw() { if(void* ptr = __kernel_frame_tag->allocate(count, static_cast<size_t>(al))) return ptr; else if(std::new_handler h = std::get_new_handler()) h(); else throw std::bad_alloc{}; return __kernel_frame_tag->allocate(count, static_cast<size_t>(al)); }
[[nodiscard]] void* operator new[](std::size_t count, std::align_val_t al) throw() { if(void* ptr = __kernel_frame_tag->allocate(count, static_cast<size_t>(al))) return ptr; else if(std::new_handler h = std::get_new_handler()) h(); else throw std::bad_alloc{}; return __kernel_frame_tag->allocate(count, static_cast<size_t>(al)); }
