...
[[nodiscard]] void* operator new(std::size_t count) throw() 
{ 
    if(void* ptr = __kernel_frame_tag->allocate(count)) return ptr;
    else if(std::new_handler h = std::get_new_handler()) h();
    else throw std::bad_alloc{};
    return __kernel_frame_tag->allocate(count);
}
[[nodiscard]] void* operator new[](std::size_t count) throw() 
{
    if(void* ptr = __kernel_frame_tag->allocate(count)) return ptr;
...
[[nodiscard]] void* operator new(std::size_t count, std::align_val_t al) throw() 
{
    if(void* ptr = __kernel_frame_tag->allocate(count, static_cast<size_t>(al))) return ptr;
    else if(std::new_handler h = std::get_new_handler()) h();
    else throw std::bad_alloc{};
    return __kernel_frame_tag->allocate(count, static_cast<size_t>(al));
}
[[nodiscard]] void* operator new[](std::size_t count, std::align_val_t al) throw() 
{
    if(void* ptr = __kernel_frame_tag->allocate(count, static_cast<size_t>(al))) return ptr;
...
[[nodiscard]] void* operator new(size_t count, std::nothrow_t const&) noexcept { try { return operator new(count); } catch(std::bad_alloc& e) { return nullptr; } }
[[nodiscard]] void* operator new[](size_t count, std::nothrow_t const&) noexcept { try { return operator new[](count); } catch(std::bad_alloc& e) { return nullptr; } }
[[nodiscard]] void* operator new(size_t count, std::align_val_t al, std::nothrow_t const&) noexcept { try { return operator new(count); } catch(std::bad_alloc& e) { return nullptr; } }
[[nodiscard]] void* operator new[](size_t count, std::align_val_t al, std::nothrow_t const&) noexcept { try { return operator new[](count); } catch(std::bad_alloc& e) { return nullptr; } }
...
namespace std
{
    void* __detail::__aligned_reallocate(void* ptr, size_t n, size_t align) { return __kernel_frame_tag->reallocate(ptr, n, align); }
