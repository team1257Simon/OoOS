...
long double _Complex cpowl(long double _Complex a, long double _Complex z)
{
    ...
    y    = cimagl(z);
    absa = cabsl(a);
    if(absa == 0.0L) { return (0.0L + 0.0L * 1.0fi); }
    arga  = cargl(a);
    ...
    w = r * __builtin_cosl(theta) + (r * __builtin_sinl(theta)) * 1.0fi;
    return w;
}
