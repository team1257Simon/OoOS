...
long double                  _ctansl(long double _Complex);
long double _Complex ctanl(long double _Complex z)
{
    ...
    if(__builtin_fabsl(d) < 0.25L) d = _ctansl(z);
    if(d == 0.0L)
    {
        w = (__builtin_huge_vall()) + (__builtin_huge_vall()) * 1.0fi;
        return w;
    }
    w = __builtin_sinl(2.0L * creall(z)) / d + (__builtin_sinhl(2.0L * cimagl(z)) / d) * 1.0fi;
    return w;
}
