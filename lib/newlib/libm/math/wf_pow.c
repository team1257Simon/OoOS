...
float                    powf(float x, float y)
{
    ...
    if(__fdlib_version == __fdlibm_ieee || (__builtin_isnan(y))) return z;
    if((__builtin_isnan(x)))
    {
        if(y == (float)0.0)
        {
            ...
            exc.arg2   = (double)y;
            exc.retval = 1.0;
            if(__fdlib_version == __fdlibm_ieee || __fdlib_version == __fdlibm_posix)
                exc.retval = 1.0;
            else if(!matherr(&exc)) { (*__errno()) = 33; }
            if(exc.err != 0) (*__errno()) = exc.err;
        ...
        if(finitef(x) && finitef(y))
        {
            ...
            else
            {
                ...
                if(__fdlib_version == __fdlibm_svid)
                {
                    ...
                    y *= 0.5;
                    if(x < 0.0f && rintf(y) != y) exc.retval = -((float)3.40282346638528860e+38);
                }
                else
                {
                    exc.retval = (__builtin_huge_val());
                    y *= 0.5;
                    if(x < 0.0f && rintf(y) != y) exc.retval = -(__builtin_huge_val());
                }
                if(__fdlib_version == __fdlibm_posix)
                    (*__errno()) = 34;
                else if(!matherr(&exc)) { (*__errno()) = 34; }
                if(exc.err != 0) (*__errno()) = exc.err;
                return (float)exc.retval;
            }
        }
