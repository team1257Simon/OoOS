...
double __ieee754_fmod(double x, double y)
{
    ...
    hx ^= sx;
    hy &= 0x7fffffff;
    if((hy | ly) == 0 || (hx >= 0x7ff00000) || ((hy | ((ly | -ly) >> 31)) > 0x7ff00000)) return (x * y) / (x * y);
    if(hx <= hy)
    {
        if((hx < hy) || (lx < ly)) return x;
        if(lx == ly) return Zero[(__uint32_t)sx >> 31];
    }
    if(hx < 0x00100000)
    {
        if(hx == 0)
    ...
    while(n--)
    {
        ...
        {
            if((hz | lz) == 0) return Zero[(__uint32_t)sx >> 31];
            hx = hz + hz + (lz >> 31);
            lx = lz + lz;
        }
    }
    ...
    if((hx | lx) == 0) return Zero[(__uint32_t)sx >> 31];
    while(hx < 0x00100000)
    {
        hx = hx + hx + (lx >> 31);
        lx = lx + lx;
