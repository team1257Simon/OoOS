...
static const float two25 = 3.355443200e+07, twom25 = 2.9802322388e-08, huge = 1.0e+30, tiny = 1.0e-30;
float              scalblnf(float x, long int n)
{
    ...
    k = k + n;
    if(n > 50000 || k > 0xfe) return huge * copysignf(huge, x);
    if(n < -50000) return tiny * copysignf(tiny, x);
    if(k > 0)
    {
        do {
    ...
    if(k <= -25) return tiny * copysignf(tiny, x);
    k += 25;
    do {
        ieee_float_shape_type sf_u;
        sf_u.word = ((ix & 0x807fffff) | (k << 23));
