...
int              __tzcalc_limits(int year)
{
    ...
    tz->__tzyear = year;
    years        = (year - 1970);
    year_days    = years * 365 + (years - 1 + 2) / 4 - (years - 1 + 70) / 100 + (years - 1 + 370) / 400;
    for(i = 0; i < 2; ++i)
    {
        if(tz->__tzrule[i].ch == 'J')
        {
            days = year_days + tz->__tzrule[i].d + (((((year) % 4) == 0 && ((year) % 100) != 0) || ((year) % 400) == 0) && tz->__tzrule[i].d >= 60);
            --days;
        }
        ...
        else
        {
            ...
            while(m_day >= ip[j - 1]) m_day -= 7;
            days += m_day;
        }
        tz->__tzrule[i].change = days * ((60L * 60L) * 24L) + tz->__tzrule[i].s + tz->__tzrule[i].offset;
    }
