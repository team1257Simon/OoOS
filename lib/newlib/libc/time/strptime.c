...
char*         strptime_l(const char* restrict, const char* restrict, struct tm* restrict, locale_t);
void          tzset(void);
...
static int                                    match_string(const char* restrict* buf, const char* const* strs, locale_t locale)
{
    int i = 0;
...
char* strptime_l(const char* buf, const char* format, struct tm* timeptr, locale_t locale)
{
    char                    c;
    int                     ymd                = 0;
    ...
    for(; (c = *format) != '\0'; ++format)
    {
        char* s;
        int   ret;
        if((((_ctype_ + sizeof(""[(unsigned char)c]))[(int)((unsigned char)c)]) & 010))
        {
            while((((_ctype_ + sizeof(""[(unsigned char)*buf]))[(int)((unsigned char)*buf)]) & 010)) ++buf;
        }
        else if(c == '%' && format[1] != '\0')
        {
            ...
            case 'F':
                ...
                if(s == ((void*)0) || s == buf) return ((void*)0);
                buf = s;
                ymd |= (4 | 2 | 1);
                break;
            case 'H':
            ...
            case 's':
            {
                ...
                t            = sec;
                if(s == buf || (*__errno()) != 0 || t != sec || localtime_r(&t, timeptr) != timeptr) return ((void*)0);
                (*__errno()) = save_errno;
                buf          = s;
                ymd |= 16 | 8 | (4 | 2 | 1);
                break;
    ...
    if((ymd & (4 | 2 | 1)) == (4 | 2 | 1))
    {
        if(!(ymd & 16))
        {
            ...
            if(!is_leap_year(timeptr->tm_year + tm_year_base) || timeptr->tm_mon < 2) { timeptr->tm_yday--; }
            ymd |= 16;
        }
    }
    else if((ymd & (4 | 16)) == (4 | 16))
    {
        ...
        if(!(ymd & 1))
        {
            ...
            if(!is_leap_year(timeptr->tm_year + tm_year_base) || timeptr->tm_mon < 2) { timeptr->tm_mday++; }
        }
    }
    if((ymd & (4 | 16 | 8)) == (4 | 16))
    {
...
char* strptime(const char* buf, const char* format, struct tm* timeptr) { return strptime_l(buf, format, timeptr, __get_current_locale()); }
