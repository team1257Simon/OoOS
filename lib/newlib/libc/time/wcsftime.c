...
static size_t __strftime(wchar_t* s, size_t maxsize, const wchar_t* format, const struct tm* tim_p, struct __locale_t* locale)
{
    size_t                  count = 0;
    int                     len   = 0;
    const wchar_t*          ctloc;
    ...
    for(;;)
    {
        ...
        pad   = '\0';
        width = 0;
        if(*format == L'0' || *format == L'+') pad = *format++;
        ...
        alt = L'\0';
        if(*format == L'E') { alt = *format++; }
        else if(*format == L'O') { alt = *format++; }
        switch(*format)
        {
        case L'a':
        ...
        case L'B':
            ...
            break;
        case L'c': (ctloc = __ctloc(ctlocbuf, _CurrentTimeLocale->c_fmt, &ctloclen)); goto recurse;
        case L'r': (ctloc = __ctloc(ctlocbuf, _CurrentTimeLocale->ampm_fmt, &ctloclen)); goto recurse;
        case L'x': (ctloc = __ctloc(ctlocbuf, _CurrentTimeLocale->x_fmt, &ctloclen)); goto recurse;
        case L'X':
        ...
        {
            {
                ...
                {
                    fmt = L"%s%0.*d";
                    if(century >= 100 && pad == L'+') pos = "+";
                }
                if(width < 2) width = 2;
                len = swprintf(&s[count], maxsize - count, fmt, neg ? "-" : pos, width - neg, century);
            }
        ...
        case L'd':
        case L'e':
            len = swprintf(&s[count], maxsize - count, *format == L'd' ? L"%.2d" : L"%2d", tim_p->tm_mday);
            if(len < 0 || (count += len) >= maxsize) return 0;
            break;
        case L'D':
            len = swprintf(&s[count], maxsize - count, L"%.2d/%.2d/%.2d", tim_p->tm_mon + 1, tim_p->tm_mday,
        ...
        case L'g':
        {
            ...
            if(adjust < 0 && tim_p->tm_year <= -1900)
                ...
                adjust = -1;
            len = swprintf(&s[count], maxsize - count, L"%.2d", ((year + adjust) % 100 + 100) % 100);
            if(len < 0 || (count += len) >= maxsize) return 0;
        }
        break;
        case L'G':
        ...
        case L'H':
        case L'k':
            len = swprintf(&s[count], maxsize - count, *format == L'k' ? L"%2d" : L"%.2d", tim_p->tm_hour);
            if(len < 0 || (count += len) >= maxsize) return 0;
            break;
        case L'l':
            if(alt == L'O') alt = L'\0';
        case L'I':
        {
            ...
            len = swprintf(&s[count], maxsize - count, *format == L'I' ? L"%.2d" : L"%2d", h12);
            if(len < 0 || (count += len) >= maxsize) return 0;
        }
        break;
        case L'j':
            len = swprintf(&s[count], maxsize - count, L"%.3d", tim_p->tm_yday + 1);
            if(len < 0 || (count += len) >= maxsize) return 0;
            break;
        case L'm':
            len = swprintf(&s[count], maxsize - count, L"%.2d", tim_p->tm_mon + 1);
            if(len < 0 || (count += len) >= maxsize) return 0;
            break;
        case L'M':
            len = swprintf(&s[count], maxsize - count, L"%.2d", tim_p->tm_min);
            if(len < 0 || (count += len) >= maxsize) return 0;
            break;
        case L'n':
            if(count < maxsize - 1)
        ...
        case L'P':
            ...
            {
                if(count < maxsize - 1)
                    s[count++] = (*format == L'P' ? towlower((wint_t)(ctloc[i])) : ctloc[i]);
                else
                    return 0;
            }
            break;
        case L'R':
            len = swprintf(&s[count], maxsize - count, L"%.2d:%.2d", tim_p->tm_hour, tim_p->tm_min);
            if(len < 0 || (count += len) >= maxsize) return 0;
            break;
        case L's':
        {
        ...
        break;
        case L'S':
            len = swprintf(&s[count], maxsize - count, L"%.2d", tim_p->tm_sec);
            if(len < 0 || (count += len) >= maxsize) return 0;
            break;
        case L't':
            if(count < maxsize - 1)
            ...
            break;
        case L'T':
            len = swprintf(&s[count], maxsize - count, L"%.2d:%.2d:%.2d", tim_p->tm_hour, tim_p->tm_min, tim_p->tm_sec);
            if(len < 0 || (count += len) >= maxsize) return 0;
            break;
        case L'u':
            if(count < maxsize - 1)
            ...
            break;
        case L'U':
            len = swprintf(&s[count], maxsize - count, L"%.2d", (tim_p->tm_yday + 7 - tim_p->tm_wday) / 7);
            if(len < 0 || (count += len) >= maxsize) return 0;
            break;
        case L'V':
        {
        ...
        case L'y':
        {
            {
                int year = tim_p->tm_year >= 0 ? tim_p->tm_year % 100 : abs(tim_p->tm_year + 1900) % 100;
                len      = swprintf(&s[count], maxsize - count, L"%.2d", year);
            }
            if(len < 0 || (count += len) >= maxsize) return 0;
        }
        break;
        case L'Y':
        {
            ...
            *fmt++ = L'%';
            if(pad) *fmt++ = L'0';
            wcscpy((fmt), (L".*u"));
            len = swprintf(&s[count], maxsize - count, fmtbuf, width, year);
            if(len < 0 || (count += len) >= maxsize) return 0;
        }
        break;
        case L'z':
            if(tim_p->tm_isdst >= 0)
            {
                ...
                offset            = -tz->__tzrule[tim_p->tm_isdst > 0].offset;
                __tz_unlock();
                len = swprintf(&s[count], maxsize - count, L"%+03ld%.2ld", offset / (60L * 60L), labs(offset / 60L) % 60L);
                if(len < 0 || (count += len) >= maxsize) return 0;
            }
            break;
        case L'Z':
    ...
    if(maxsize) s[count] = L'\0';
    return count;
}
size_t wcsftime(wchar_t* restrict s, size_t maxsize, const wchar_t* restrict format, const struct tm* restrict tim_p)
{
    return __strftime((s), (maxsize), (format), (tim_p), (__get_current_locale()));
}
size_t wcsftime_l(wchar_t* restrict s, size_t maxsize, const wchar_t* restrict format, const struct tm* restrict tim_p, struct __locale_t* locale)
{
