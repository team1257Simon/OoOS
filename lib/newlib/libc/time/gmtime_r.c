...
struct tm*       gmtime_r(const time_t* restrict tim_p, struct tm* restrict res)
{
    ...
    days = lcltime / ((60L * 60L) * 24L) + 719468L;
    rem  = lcltime % ((60L * 60L) * 24L);
    if(rem < 0)
    {
        rem += ((60L * 60L) * 24L);
        --days;
    ...
    if((weekday = ((3 + days) % 7)) < 0) weekday += 7;
    res->tm_wday = weekday;
    era          = (days >= 0 ? days : days - (146097L - 1)) / 146097L;
    eraday       = days - era * 146097L;
    erayear      = (eraday - eraday / ((3 * 365 + 366) - 1) + eraday / 36524L - eraday / (146097L - 1)) / 365;
    ...
    res->tm_yday  = yearday >= 365 - 31 - 28 ? yearday - (365 - 31 - 28) :
