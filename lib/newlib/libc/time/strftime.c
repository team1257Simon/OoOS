...

{
 ...
 case 'c':
     ...
     (ctloclen = strlen (ctloc = _CurrentTimeLocale->c_fmt));
   goto recurse;
 case 'r':
   (ctloclen = strlen (ctloc = _CurrentTimeLocale->ampm_fmt));
   goto recurse;
 case 'x':
...
recurse:
 ...
 case 'C':
  ...
  if (width < 2)
    width = 2;
  len = sniprintf (&s[count], maxsize - count, fmt,
 ...
 case 'd':
 case 'e':
   len = sniprintf (&s[count], maxsize - count,
     *format == 'd' ? "%.2d" : "%2d",
     tim_p->tm_mday);
   if (len < 0 || (count += len) >= maxsize) return 0;
   break;
 ...
 case 'g':
  ...
  adjust = -1;
     len = sniprintf (&s[count], maxsize - count, "%.2d",
 ...
 case 'I':
   ...
   break;
 case 'j':
   len = sniprintf (&s[count], maxsize - count, "%.3d",
     tim_p->tm_yday + 1);
          if (len < 0 || (count += len) >= maxsize) return 0;
   break;
 case 'm':
 ...
 case 'P':
  ...
  return 0;
   ...
   break;
 case 'R':
          len = sniprintf (&s[count], maxsize - count, "%.2d:%.2d",
 ...
 case 't':
   ...
   break;
 case 'T':
          len = sniprintf (&s[count], maxsize - count, "%.2d:%.2d:%.2d",
  ...
  *fmt++ = '0';
       strcpy((fmt),(".*u"));
       len = sniprintf (&s[count], maxsize - count, fmtbuf, width,
         year);
       if (len < 0 || (count += len) >= maxsize) return 0;
     }
   break;
 case 'z':
       ...
       if (!tzset_called)
  {
    ...
    tzset_called = 1;
       ...
       __tz_unlock();
       len = sniprintf (&s[count], maxsize - count, "%+03ld%.2ld",
         offset / (60L * 60L),
         labs (offset / 60L) % 60L);
              if (len < 0 || (count += len) >= maxsize) return 0;
            }
