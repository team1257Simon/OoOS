...
{
    int leap = ((((tim_p->tm_year + (1900 - (tim_p->tm_year < 0 ? 0 : 2000))) % 4) == 0 &&
                 ((tim_p->tm_year + (1900 - (tim_p->tm_year < 0 ? 0 : 2000))) % 100) != 0) ||
                ((tim_p->tm_year + (1900 - (tim_p->tm_year < 0 ? 0 : 2000))) % 400) == 0);
    ...
    case(((365) << 4) + (1 << 1) + (1)):
    case(((365) << 4) + (2 << 1) + (1)):
    case(((365) << 4) + (3 << 1) + (1)): return 1;
    }
    return 0;
}
static size_t __strftime(char* s, size_t maxsize, const char* format, const struct tm* tim_p, struct __locale_t* locale)
{
    size_t                  count = 0;
    int                     len   = 0;
    const char*             ctloc;
    ...
    for(;;)
    {
        ...
        alt = '\0';
        if(*format == 'E') { alt = *format++; }
        else if(*format == 'O') { alt = *format++; }
        switch(*format)
        {
        case 'a':
            (ctloclen = strlen(ctloc = _CurrentTimeLocale->wday[tim_p->tm_wday]));
            ...
            break;
        case 'A':
            (ctloclen = strlen(ctloc = _CurrentTimeLocale->weekday[tim_p->tm_wday]));
            for(i = 0; i < ctloclen; i++)
            {
                if(count < maxsize - 1)
                    s[count++] = ctloc[i];
        ...
        case 'b':
        case 'h':
            (ctloclen = strlen(ctloc = _CurrentTimeLocale->mon[tim_p->tm_mon]));
            for(i = 0; i < ctloclen; i++)
            {
                if(count < maxsize - 1)
                    s[count++] = ctloc[i];
            ...
            break;
        case 'B':
            (ctloclen = strlen(ctloc = _CurrentTimeLocale->month[tim_p->tm_mon]));
            for(i = 0; i < ctloclen; i++)
            {
                if(count < maxsize - 1)
                    s[count++] = ctloc[i];
            ...
            break;
        case 'c': (ctloclen = strlen(ctloc = _CurrentTimeLocale->c_fmt)); goto recurse;
        case 'r': (ctloclen = strlen(ctloc = _CurrentTimeLocale->ampm_fmt)); goto recurse;
        case 'x': (ctloclen = strlen(ctloc = _CurrentTimeLocale->x_fmt)); goto recurse;
        case 'X':
            (ctloclen = strlen(ctloc = _CurrentTimeLocale->X_fmt));
        recurse:
            if(*ctloc)
            {
        ...
        {
            {
                ...
                int   century = tim_p->tm_year >= 0 ? tim_p->tm_year / 100 + 1900 / 100 : abs(tim_p->tm_year + 1900) / 100;
                if(pad)
                {
                    fmt = "%s%0.*d";
                    if(century >= 100 && pad == '+') pos = "+";
                }
                if(width < 2) width = 2;
                len = sniprintf(&s[count], maxsize - count, fmt, neg ? "-" : pos, width - neg, century);
            }
        ...
        case 'd':
        case 'e':
            len = sniprintf(&s[count], maxsize - count, *format == 'd' ? "%.2d" : "%2d", tim_p->tm_mday);
            if(len < 0 || (count += len) >= maxsize) return 0;
            break;
        case 'D':
            len = sniprintf(&s[count], maxsize - count, "%.2d/%.2d/%.2d", tim_p->tm_mon + 1, tim_p->tm_mday,
                            tim_p->tm_year >= 0 ? tim_p->tm_year % 100 : abs(tim_p->tm_year + 1900) % 100);
            if(len < 0 || (count += len) >= maxsize) return 0;
            break;
        case 'F':
        {
            ...
            *fmt++ = '%';
            ...
            if(width)
            {
                len = sniprintf(fmt, fmtbuf + 32 - fmt, "%lu", width);
                if(len > 0) fmt += len;
            }
            strcpy((fmt), ("Y-%m-%d"));
        ...
        case 'g':
        {
            ...
            int year   = tim_p->tm_year >= 0 ? tim_p->tm_year % 100 : abs(tim_p->tm_year + 1900) % 100;
            if(adjust < 0 && tim_p->tm_year <= -1900)
                adjust = 1;
            else if(adjust > 0 && tim_p->tm_year < -1900)
                adjust = -1;
            len = sniprintf(&s[count], maxsize - count, "%.2d", ((year + adjust) % 100 + 100) % 100);
            if(len < 0 || (count += len) >= maxsize) return 0;
        }
        break;
        case 'G':
        {
            ...
            *fmt++ = '%';
            if(pad) *fmt++ = '0';
            strcpy((fmt), (".*u"));
            len = sniprintf(&s[count], maxsize - count, fmtbuf, width, p_year);
            if(len < 0 || (count += len) >= maxsize) return 0;
        }
        break;
        case 'H':
        case 'k':
            len = sniprintf(&s[count], maxsize - count, *format == 'k' ? "%2d" : "%.2d", tim_p->tm_hour);
            if(len < 0 || (count += len) >= maxsize) return 0;
            break;
        case 'l':
            if(alt == 'O') alt = '\0';
        case 'I':
        {
            ...
            len = sniprintf(&s[count], maxsize - count, *format == 'I' ? "%.2d" : "%2d", h12);
            if(len < 0 || (count += len) >= maxsize) return 0;
        }
        break;
        case 'j':
            len = sniprintf(&s[count], maxsize - count, "%.3d", tim_p->tm_yday + 1);
            if(len < 0 || (count += len) >= maxsize) return 0;
            break;
        case 'm':
            len = sniprintf(&s[count], maxsize - count, "%.2d", tim_p->tm_mon + 1);
            if(len < 0 || (count += len) >= maxsize) return 0;
            break;
        case 'M':
            len = sniprintf(&s[count], maxsize - count, "%.2d", tim_p->tm_min);
            if(len < 0 || (count += len) >= maxsize) return 0;
            break;
        case 'n':
            if(count < maxsize - 1)
        ...
        case 'p':
        case 'P':
            (ctloclen = strlen(ctloc = _CurrentTimeLocale->am_pm[tim_p->tm_hour < 12 ? 0 : 1]));
            for(i = 0; i < ctloclen; i++)
            {
                if(count < maxsize - 1)
                    s[count++] = (*format == 'P' ? __extension__({
                        __typeof__((int)(unsigned char)(ctloc[i])) __x = ((int)(unsigned char)(ctloc[i]));
            ...
            break;
        case 'R':
            len = sniprintf(&s[count], maxsize - count, "%.2d:%.2d", tim_p->tm_hour, tim_p->tm_min);
            if(len < 0 || (count += len) >= maxsize) return 0;
            break;
        case 's':
        {
            ...
            len = sniprintf(&s[count], maxsize - count, "%lld",
                            (((((long long)tim_p->tm_year - 69) / 4 - (tim_p->tm_year - 1) / 100 + (tim_p->tm_year + 299) / 400 +
        ...
        break;
        case 'S':
            len = sniprintf(&s[count], maxsize - count, "%.2d", tim_p->tm_sec);
            if(len < 0 || (count += len) >= maxsize) return 0;
            break;
        case 't':
            if(count < maxsize - 1)
            ...
            break;
        case 'T':
            len = sniprintf(&s[count], maxsize - count, "%.2d:%.2d:%.2d", tim_p->tm_hour, tim_p->tm_min, tim_p->tm_sec);
            if(len < 0 || (count += len) >= maxsize) return 0;
            break;
        case 'u':
            if(count < maxsize - 1)
            ...
            break;
        case 'U':
            len = sniprintf(&s[count], maxsize - count, "%.2d", (tim_p->tm_yday + 7 - tim_p->tm_wday) / 7);
            if(len < 0 || (count += len) >= maxsize) return 0;
            break;
        case 'V':
        {
            ...
            if(adjust > 0)
                ...
                week = 52 + (4 >= (wday - tim_p->tm_yday -
                                   ((((tim_p->tm_year + (1900 - 1 - (tim_p->tm_year < 0 ? 0 : 2000))) % 4) == 0 &&
                                     ((tim_p->tm_year + (1900 - 1 - (tim_p->tm_year < 0 ? 0 : 2000))) % 100) != 0) ||
                                    ((tim_p->tm_year + (1900 - 1 - (tim_p->tm_year < 0 ? 0 : 2000))) % 400) == 0)));
            len = sniprintf(&s[count], maxsize - count, "%.2d", week);
            if(len < 0 || (count += len) >= maxsize) return 0;
        }
        break;
        case 'w':
        ...
        case 'W':
        {
            ...
            len      = sniprintf(&s[count], maxsize - count, "%.2d", wday);
            if(len < 0 || (count += len) >= maxsize) return 0;
        }
        break;
        case 'y':
        {
            {
                int year = tim_p->tm_year >= 0 ? tim_p->tm_year % 100 : abs(tim_p->tm_year + 1900) % 100;
                len      = sniprintf(&s[count], maxsize - count, "%.2d", year);
            }
            if(len < 0 || (count += len) >= maxsize) return 0;
        }
        break;
        case 'Y':
        {
            char              fmtbuf[10], *fmt = fmtbuf;
            int               sign = tim_p->tm_year < -1900;
            register unsigned year = (unsigned)tim_p->tm_year + (unsigned)1900;
            if(sign)
            {
                *fmt++ = '-';
                year   = (0x7fffffff * 2U + 1U) - year + 1;
            }
            ...
            *fmt++ = '%';
            if(pad) *fmt++ = '0';
            strcpy((fmt), (".*u"));
            len = sniprintf(&s[count], maxsize - count, fmtbuf, width, year);
            if(len < 0 || (count += len) >= maxsize) return 0;
        }
        break;
        case 'z':
            if(tim_p->tm_isdst >= 0)
            {
                ...
                offset            = -tz->__tzrule[tim_p->tm_isdst > 0].offset;
                __tz_unlock();
                len = sniprintf(&s[count], maxsize - count, "%+03ld%.2ld", offset / (60L * 60L), labs(offset / 60L) % 60L);
                if(len < 0 || (count += len) >= maxsize) return 0;
            }
            break;
        case 'Z':
    ...
    if(maxsize) s[count] = '\0';
    return count;
}
size_t strftime(char* restrict s, size_t maxsize, const char* restrict format, const struct tm* restrict tim_p)
{
    return __strftime((s), (maxsize), (format), (tim_p), (__get_current_locale()));
}
size_t strftime_l(char* restrict s, size_t maxsize, const char* restrict format, const struct tm* restrict tim_p, struct __locale_t* locale)
{
