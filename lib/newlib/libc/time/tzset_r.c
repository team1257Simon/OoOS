...
void             _tzset_unlocked_r(struct _reent* reent_ptr)
{
    ...
    if(prev_tzenv != ((void*)0) && strcmp(tzenv, prev_tzenv) == 0) return;
    free(prev_tzenv);
    prev_tzenv = _malloc_r(reent_ptr, strlen(tzenv) + 1);
    if(prev_tzenv != ((void*)0)) strcpy(prev_tzenv, tzenv);
    if(*tzenv == ':') ++tzenv;
    if(siscanf(tzenv, "%10[^0-9,+-]%n", __tzname_std, &n) <= 0) return;
    tzenv += n;
    sign = 1;
    if(*tzenv == '-')
    {
    ...
    mm = 0;
    ss = 0;
    if(siscanf(tzenv, "%hu%n:%hu%n:%hu%n", &hh, &n, &mm, &n, &ss, &n) < 1) return;
    ...
    for(i = 0; i < 2; ++i)
    {
        ...
        if(*tzenv == 'M')
        {
            if(siscanf(tzenv, "M%hu%n.%hu%n.%hu%n", &m, &n, &w, &n, &d, &n) != 3 || m < 1 || m > 12 || w < 1 || w > 5 || d > 6) return;
            tz->__tzrule[i].ch = 'M';
            tz->__tzrule[i].m  = m;
            tz->__tzrule[i].n  = w;
            tz->__tzrule[i].d  = d;
        ...
        ss = 0;
        n  = 0;
        if(*tzenv == '/') siscanf(tzenv, "/%hu%n:%hu%n:%hu%n", &hh, &n, &mm, &n, &ss, &n);
