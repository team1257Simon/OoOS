...
struct tm*       localtime_r(const time_t* restrict tim_p, struct tm* restrict res)
{
    long                 offset;
    int                  hours, mins, secs;
    int                  year;
    ...
    if(_daylight)
    {
        if(year == tz->__tzyear || __tzcalc_limits(year))
            res->tm_isdst = (tz->__tznorth ? (*tim_p >= tz->__tzrule[0].change && *tim_p < tz->__tzrule[1].change) :
