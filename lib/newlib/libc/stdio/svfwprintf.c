...
static wchar_t*    wcvt(struct _reent*, double, int, int, wchar_t*, int*, int, int*, wchar_t*, int);
...
int                _svfwprintf_r(struct _reent* data, FILE* fp, const wchar_t* fmt0, va_list ap)
{
        ...
        {
            {
                ...
                if(++uio.uio_iovcnt >= 8)
                {
                    if(__ssprint_r(data, fp, &uio)) goto error;
                    iovp = iov;
                }
            };
            ret += m;
    ...
    reswitch:
        ...
        case L'9':
            ...
            width = n;
            goto reswitch;
        case L'L': flags |= 0x008; goto rflag;
        ...
        case L'g':
        case L'G':
            if(flags & 0x008) { (_double_.fp) = (double)(__builtin_va_arg(ap, long double)); }
            else { (_double_.fp) = (__builtin_va_arg(ap, double)); }
            if((__builtin_isinf_sign((_double_.fp))))
            {
                if((_double_.fp) < 0) sign = '-';
                if(ch <= L'G')
                    cp = L"INF";
                else
                    cp = L"inf";
            ...
            if((__builtin_isnan((_double_.fp))))
            {
                if(((sizeof((_double_.fp)) == sizeof(float))  ? __builtin_signbitf((_double_.fp)) :
                    (sizeof((_double_.fp)) == sizeof(double)) ? __builtin_signbit((_double_.fp)) :
                                                                __builtin_signbitl((_double_.fp))))
                    sign = L'-';
                if(ch <= L'G')
                    cp = L"NAN";
                else
            ...
            cp = buf;
            if(prec == -1) { prec = 6; }
            else if((ch == L'g' || ch == L'G') && prec == 0) { prec = 1; }
            flags |= 0x100;
            cp = wcvt(data, (_double_.fp), prec, flags, &softsign, &expt, ch, &ndig, cp, 40);
            if(cp == buf && ndig > 40 && malloc_buf == ((void*)0))
            {
                if((malloc_buf = (wchar_t*)_malloc_r(data, ndig * sizeof(wchar_t))) == ((void*)0))
                {
                    fp->_flags |= 0x0040;
                    goto error;
                }
                cp = wcvt(data, (_double_.fp), prec, flags, &softsign, &expt, ch, &ndig, malloc_buf, ndig);
            }
            if(ch == L'g' || ch == L'G')
            {
                if(expt <= -4 || expt > prec)
            ...
            else
            {
                if(ch == L'f')
                {
                    ...
                    {
                        size = expt;
                        if(prec || flags & 0x001) size += prec + 1;
                    }
                    else
                        size = (prec || flags & 0x001) ? prec + 2 : 1;
                }
                else if(expt >= ndig)
                {
                    size = expt;
        ...
        else
        {
            if(ch >= L'f')
            {
                if((_double_.fp) == 0)
                {
                    ...
                    {
                        {
                            iovp->iov_base = (char*)(&decimal_point);
                            iovp->iov_len  = (1) * sizeof(wchar_t);
                            uio.uio_resid += (1) * sizeof(wchar_t);
                            iovp++;
                            if(++uio.uio_iovcnt >= 8)
                ...
                else if(expt <= 0)
                {
                    ...
                    if(expt || ndig || flags & 0x001)
                    {
                        ...
                        {
                            iovp->iov_base = (char*)(cp);
                            iovp->iov_len  = (ndig) * sizeof(wchar_t);
                            uio.uio_resid += (ndig) * sizeof(wchar_t);
                            iovp++;
                            if(++uio.uio_iovcnt >= 8)
                            {
                                if(__ssprint_r(data, fp, &uio)) goto error;
            ...
            else
            {
                if(ndig > 1 || flags & 0x001)
                {
                    ...
                    if((_double_.fp))
                    {
                        {
                            iovp->iov_base = (char*)(cp);
                            iovp->iov_len  = (ndig - 1) * sizeof(wchar_t);
                            uio.uio_resid += (ndig - 1) * sizeof(wchar_t);
                            iovp++;
                            if(++uio.uio_iovcnt >= 8)
                            {
                                if(__ssprint_r(data, fp, &uio)) goto error;
                ...
                {
                    iovp->iov_base = (char*)(expstr);
                    iovp->iov_len  = (expsize) * sizeof(wchar_t);
                    uio.uio_resid += (expsize) * sizeof(wchar_t);
                    iovp++;
                    if(++uio.uio_iovcnt >= 8)
                    {
                        if(__ssprint_r(data, fp, &uio)) goto error;
        ...
        {
            if(uio.uio_resid && __ssprint_r(data, fp, &uio)) goto error;
            uio.uio_iovcnt = 0;
            iovp           = iov;
        };
...
static wchar_t* wcvt(struct _reent* data, double value, int ndigits, int flags, wchar_t* sign, int* decpt, int ch, int* length, wchar_t* buf, int len)
{
    int                mode, dsgn;
    ...
    if((tmp.i[1]) & ((__uint32_t)0x80000000L))
    {
        ...
        *sign = L'\0';
    if(ch == L'f' || ch == L'F') { mode = 3; }
    else
    {
        if(ch == L'e' || ch == L'E') { ndigits++; }
        mode = 2;
    }
    {
        char *digits, *bp, *rve;
