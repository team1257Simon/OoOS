...
static _fpos_t funseeker(struct _reent* ptr, void* cookie, _fpos_t off, int whence)
{
    ...
    fpos_t     result;
    (*__errno()) = 0;
    if((result = c->seekfn(c->cookie, (fpos_t)off, whence)) < 0 && (*__errno())) ptr->_errno = (*__errno());
    return result;
}
...
FILE* _funopen_r(struct _reent* ptr, const void* cookie, funread readfn, funwrite writefn, funseek seekfn, funclose closefn)
{
    FILE*      fp;
    funcookie* c;
    if(!readfn && !writefn)
