...
char* _tempnam_r(struct _reent* p, const char* dir, const char* pfx)
{
    ...
    if(dir == ((void*)0) && (dir = getenv("TMPDIR")) == ((void*)0)) dir = "/tmp";
    length   = strlen(dir) + strlen(prefix) + (4 * sizeof(int)) + 2 + 1;
    filename = _malloc_r(p, length);
    if(filename)
    {
        if(!worker(p, filename, dir, prefix, _getpid_r(p) ^ (int)(long)p, &p->_inc)) return ((void*)0);
    }
    return filename;
}
char* tempnam(const char* dir, const char* pfx) { return _tempnam_r(_impure_ptr, dir, pfx); }
char* tmpnam(char* s) { return _tmpnam_r(_impure_ptr, s); }
