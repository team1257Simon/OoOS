...
static __inline__ size_t __fbufsize(FILE* __fp) { return (size_t)__fp->_bf._size; }
static __inline__ int    __freading(FILE* __fp) { return (__fp->_flags & 0x0004) != 0; }
static __inline__ int    __fwriting(FILE* __fp) { return (__fp->_flags & 0x0008) != 0; }
static __inline__ int    __freadable(FILE* __fp) { return (__fp->_flags & (0x0004 | 0x0010)) != 0; }
static __inline__ int    __fwritable(FILE* __fp) { return (__fp->_flags & (0x0008 | 0x0010)) != 0; }
static __inline__ int    __flbf(FILE* __fp) { return (__fp->_flags & 0x0001) != 0; }
static __inline__ size_t __fpending(FILE* __fp) { return __fp->_p - __fp->_bf._base; }
struct stat;
struct tms;
struct timeval;
struct timezone;
...
int                     __fsetlocking(FILE* fp, int type)
{
    int result;
    do {
        struct _reent* _check_init_ptr = (_impure_ptr);
        if((_check_init_ptr) && !(_check_init_ptr)->__sdidinit) __sinit(_check_init_ptr);
    } while(0);
    result = (fp->_flags2 & 0x0001) ? 2 : 1;
    switch(type)
    {
    case 2: fp->_flags2 |= 0x0001; break;
    case 1: fp->_flags2 &= ~0x0001; break;
    case 0:
    default: break;
    }
    return result;
