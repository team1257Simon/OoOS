...
extern 
      int 
                              __sread (struct _reent *, void *, char *,
            
           int
                                   );
extern 
      int 
                              __seofread (struct _reent *, void *,
        char *,
 ...
 fp
  ...
  if (s != 
          ((void *)0) 
               && *s != '\0')
    {
      { const char *p = (s); size_t len = strlen (p); while (len) { ssize_t len1 = _write_r (ptr, fileno (fp), p, len); if (len1 < 0) break; len -= len1; p += len1; } };
      { const char *p = (": "); size_t len = strlen (p); while (len) { ssize_t len1 = _write_r (ptr, fileno (fp), p, len); if (len1 < 0) break; len -= len1; p += len1; } };
    }

  if ((error = _strerror_r (ptr, ptr->_errno, 1, &dummy)) != 
                                                            ((void *)0)
                                                                )
    { const char *p = (error); size_t len = strlen (p); while (len) { ssize_t len1 = _write_r (ptr, fileno (fp), p, len); if (len1 < 0) break; len -= len1; p += len1; } };




  { const char *p = ("\n"); size_t len = strlen (p); while (len) { ssize_t len1 = _write_r (ptr, fileno (fp), p, len); if (len1 < 0) break; len -= len1; p += len1; } };
