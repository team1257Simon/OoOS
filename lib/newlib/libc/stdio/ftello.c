...
_off_t                  _ftello_r(struct _reent* ptr, register FILE* fp)
{
    ...
    } while(0);
    {
        ...
        if(!(fp->_flags & 0x0004) && (fp->_flags & 0x0008) && fp->_p != ((void*)0) && fp->_p - fp->_bf._base > 0 && (fp->_flags & 0x0100))
        {
            pos = fp->_seek(ptr, fp->_cookie, (_fpos_t)0, 2);
            if(pos == (_fpos_t)-1)
            {
                if(!(fp->_flags2 & 0x0001)) (((fp)->_flags & 0x0200) ? 0 : ((void)0));
                ;
        ...
        {
            pos -= fp->_r;
            if(((fp)->_ub._base != ((void*)0))) pos -= fp->_ur;
        }
        else if((fp->_flags & 0x0008) && fp->_p != ((void*)0)) { pos += fp->_p - fp->_bf._base; }
        if(!(fp->_flags2 & 0x0001)) (((fp)->_flags & 0x0200) ? 0 : ((void)0));
    };
    return (_off_t)pos;
}
_off_t ftello(register FILE* fp) { return _ftello_r(_impure_ptr, fp); }
