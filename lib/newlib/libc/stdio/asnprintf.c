...
char*                   _asnprintf_r(struct _reent* restrict ptr, char* buf, size_t* lenp, const char* restrict fmt, ...)
{
    int     ret;
    va_list ap;
    FILE    f;
    size_t  len = *lenp;
    if(buf && len) { f._flags = 0x0008 | 0x0200 | 0x0400; }
    else
    {
...
char* asnprintf(char* restrict buf, size_t* restrict lenp, const char* restrict fmt, ...)
{
    int            ret;
    va_list        ap;
    FILE           f;
