...
FILE*                   _freopen_r(struct _reent* ptr, const char* restrict file, const char* restrict mode, register FILE* restrict fp)
{
    register int f;
    ...
    oflags2 = fp->_flags2;
    if(!(oflags2 & 0x0001)) (((fp)->_flags & 0x0200) ? 0 : ((void)0));
    ...
    {
        if(fp->_flags & 0x0008) _fflush_r(ptr, fp);
        if(fp->_close != ((void*)0) && file != ((void*)0)) fp->_close(ptr, fp->_cookie);
    }
    if(file != ((void*)0))
    {
    ...
    else
    {
        ...
        {
            e = 9;
            if(fp->_close != ((void*)0)) fp->_close(ptr, fp->_cookie);
        }
    }
    if(fp->_flags & 0x0080) _free_r(ptr, (char*)fp->_bf._base);
    fp->_w        = 0;
    fp->_r        = 0;
    fp->_p        = ((void*)0);
    fp->_bf._base = ((void*)0);
...
FILE* freopen(const char* restrict file, const char* restrict mode, register FILE* restrict fp) { return _freopen_r(_impure_ptr, file, mode, fp); }
