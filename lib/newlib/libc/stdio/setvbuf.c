...
int                     setvbuf(register FILE* fp, char* buf, register int mode, register size_t size)
{
    ...
    if(mode != 2)
        if((mode != 0 && mode != 1) || (int)(long)size < 0) return ((-1));
    {
        if(!(fp->_flags2 & 0x0001)) (((fp)->_flags & 0x0200) ? 0 : ((void)0));
        _fflush_r(reent, fp);
        if(((fp)->_ub._base != ((void*)0)))
        {
            if((fp)->_ub._base != (fp)->_ubuf) _free_r(reent, (char*)(fp)->_ub._base);
            (fp)->_ub._base = ((void*)0);
        };
        fp->_r = fp->_lbfsize = 0;
        if(fp->_flags & 0x0080) _free_r(reent, (void*)fp->_bf._base);
        ...
        if(size != iosize) fp->_flags |= 0x0800;
        if(mode == 1) fp->_flags |= 0x0001;
