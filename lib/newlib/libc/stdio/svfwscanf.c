...
{
    if(wc == ((wint_t)-1)) return (((wint_t)-1));
    fp->_flags &= ~0x0020;
    if(((fp)->_ub._base != ((void*)0)))
    {
        if(fp->_r >= fp->_ub._size && __submore(data, fp)) { return (-1); }
        fp->_p -= sizeof(wchar_t);
        *fp->_p = (wchar_t)wc;
        fp->_r += sizeof(wchar_t);
        return wc;
    }
    if(fp->_bf._base != ((void*)0) && fp->_p > fp->_bf._base && ((wchar_t*)fp->_p)[-1] == wc)
    {
        fp->_p -= sizeof(wchar_t);
        fp->_r += sizeof(wchar_t);
        return wc;
...
{
    wchar_t wc;
    if(fp->_r <= 0 && __ssrefill_r(ptr, fp)) return (((wint_t)-1));
    wc = *(wchar_t*)fp->_p;
    fp->_p += sizeof(wchar_t);
    fp->_r -= sizeof(wchar_t);
    return (wc);
}
int __ssvfwscanf_r(struct _reent* rptr, register FILE* fp, wchar_t const* fmt0, va_list ap)
{
    ...
    for(;;)
    {
        ...
        if(iswspace(c))
        {
            while((c = _sfgetwc_r(rptr, fp)) != ((wint_t)-1) && iswspace(c))
                ;
            if(c != ((wint_t)-1)) _sungetwc_r(rptr, c, fp);
            continue;
        }
        if(c != L'%') goto literal;
        width = 0;
    ...
    again:
        ...
        case L'%':
        literal:
            if((wi = _sfgetwc_r(rptr, fp)) == ((wint_t)-1)) goto input_failure;
            if(wi != c)
            {
        ...
        if((flags & 0x40) == 0)
        {
            while((wi = _sfgetwc_r(rptr, fp)) != ((wint_t)-1) && iswspace(wi)) nread++;
        ...
        case 1:
            ...
            {
                n = 0;
                while((wi = _sfgetwc_r(rptr, fp)) != ((wint_t)-1) && width-- != 0 &&
                      (cclcompl ? (wmemchr(ccls, (wi), ccle - ccls) == ((void*)0)) : (wmemchr(ccls, (wi), ccle - ccls) != ((void*)0))))
                    n++;
                if(wi != ((wint_t)-1)) _sungetwc_r(rptr, wi, fp);
        ...
        case 2:
            ...
            if((flags & 0x10) && (flags & 0x01))
            {
                while((wi = _sfgetwc_r(rptr, fp)) != ((wint_t)-1) && width-- != 0 && !iswspace(wi)) nread++;
                if(wi != ((wint_t)-1)) _sungetwc_r(rptr, wi, fp);
            }
            else if(flags & 0x01)
            {
                p0 = p = (__builtin_va_arg(ap, wchar_t*));
                while((wi = _sfgetwc_r(rptr, fp)) != ((wint_t)-1) && width-- != 0 && !iswspace(wi))
                {
                    *p++ = (wchar_t)wi;
                    nread++;
                }
        ...
        case 3:
        {
            ...
            if(flags & 0x100)
            {
                if(p > buf) _sungetwc_r(rptr, *--p, fp);
                goto match_failure;
            }
            c = p[-1];
            if(c == L'x' || c == L'X')
