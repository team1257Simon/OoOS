...
FILE*   funopen(const void* __cookie, int (*__readfn)(void* __cookie, char* __buf, int __n),
...
int                     _fclose_r(struct _reent* rptr, register FILE* fp)
{
    ...
    do {
        struct _reent* _check_init_ptr = (rptr);
    ...
    if(fp->_close != ((void*)0) && fp->_close(rptr, fp->_cookie) < 0) r = (-1);
    if(fp->_flags & 0x0080) _free_r(rptr, (char*)fp->_bf._base);
    if(((fp)->_ub._base != ((void*)0)))
    {
        if((fp)->_ub._base != (fp)->_ubuf) _free_r(rptr, (char*)(fp)->_ub._base);
        (fp)->_ub._base = ((void*)0);
    };
    if(((fp)->_lb._base != ((void*)0)))
    {
        _free_r(rptr, (char*)(fp)->_lb._base);
        (fp)->_lb._base = ((void*)0);
    };
    __sfp_lock_acquire();
    fp->_flags = 0;
...
int fclose(register FILE* fp) { return _fclose_r(_impure_ptr, fp); }
