...
int __ssvfscanf_r(struct _reent* rptr, register FILE* fp, char const* fmt0, va_list ap)
{
    ...
    for(;;)
    {
        ...
        if(nbytes == 1 && ((((_ctype_) + sizeof(""[wc]))[(int)(wc)]) & 010))
        {
            for(;;)
            {
                if((fp->_r <= 0 && __ssrefill_r(rptr, fp)) || !((((_ctype_) + sizeof(""[*fp->_p]))[(int)(*fp->_p)]) & 010)) break;
                nread++, fp->_r--, fp->_p++;
            }
            continue;
        }
    ...
    again:
        ...
        literal:
            ...
            for(n = 0; n < nbytes; ++n)
            {
                if((fp->_r <= 0 && __ssrefill_r(rptr, fp))) goto input_failure;
            ...
            if(flags & 0x01)
            {
                ...
                while(width != 0)
                {
                    ...
                    if(mbslen != (size_t)-2)
                    {
                        ...
                        {
                            while(n != 0) _sungetc_r(rptr, (unsigned char)buf[--n], fp);
                            break;
                        }
                        nread += n;
                        if(mbslen != 3 || state.__count != 4) width -= 1;
        ...
        case 2:
            ...
            else if(flags & 0x10)
            {
                ...
                while(!((((_ctype_) + sizeof(""[*fp->_p]))[(int)(*fp->_p)]) & 010))
                {
                    n++, fp->_r--, fp->_p++;
                    if(--width == 0) break;
                    if((fp->_r <= 0 && __ssrefill_r(rptr, fp))) break;
                }
                nread += n;
            }
            else
