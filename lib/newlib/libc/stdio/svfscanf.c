...
int
__ssvfscanf_r (struct _reent *rptr,
       register FILE *fp,
       char const *fmt0,
       va_list ap)
{
 ...
 case 4:
 {
       ...
       if ((flags & (0x200 | 0x400)) == 0x400)
  {
    ...
    exp_start = p;
                ...
                new_exp = _strtol_r (rptr, (exp_start + 1), 
                                                           ((void *)0)
                                                               , 10) - exp_adjust;
       if (exp_adjust)
  {
    ...
    if (exp_start >= buf + sizeof (buf) - ((8 
                                         * sizeof (long) - 1) * 4 / 13 + 2))
      exp_start = buf + sizeof (buf) - ((8 
                                      * sizeof (long) - 1) * 4 / 13 + 2) - 1;
                 sprintf (exp_start, "e%ld", new_exp);
  }
         res = _strtod_r (rptr, buf, 
                                    ((void *)0)
                                        );
