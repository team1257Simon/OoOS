...
int                     __sflush_r(struct _reent* ptr, register FILE* fp)
{
    ...
    if((flags & 0x0008) == 0)
    {
        ...
        if((fp->_r > 0 || fp->_ur > 0) && fp->_seek != ((void*)0))
        {
            ...
            if(curoff != -1 || ptr->_errno == 0 || ptr->_errno == 29 || ptr->_errno == 22)
            {
                fp->_flags &= ~0x0800;
                fp->_r = 0;
                fp->_p = fp->_bf._base;
                if((fp->_flags & 0x1000) && (curoff != -1 || ptr->_errno == 0)) fp->_offset = curoff;
                ptr->_errno = tmp_errno;
    ...
    if((p = fp->_bf._base) == ((void*)0)) { return 0; }
    n      = fp->_p - p;
    fp->_p = p;
...
{
    if(fp == ((void*)0)) return _fwalk_reent(_global_impure_ptr, _fflush_r);
