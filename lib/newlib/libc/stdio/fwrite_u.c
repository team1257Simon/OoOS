...
size_t     _fwrite_unlocked_r(struct _reent* ptr, const void* restrict buf, size_t size, size_t count, FILE* restrict fp)
{
    size_t        n;
    struct __suio uio;
    struct __siov iov;
...
size_t fwrite_unlocked(const void* restrict buf, size_t size, size_t count, FILE* fp)
{
    return _fwrite_unlocked_r(_impure_ptr, buf, size, count, fp);
}
