...
__attribute__((__noinline__)) static int __sbprintf(struct _reent* rptr, register FILE* fp, const char* fmt, va_list ap)
{
    int           ret;
    FILE          fake;
...
static char*   cvt(struct _reent*, double, int, int, char*, int*, int, int*, char*);
    ...
    } while(0);
    {
        ...
        reswitch:
            ...
            case ' ':
                if(!sign) sign = ' ';
                goto rflag;
            case '#': flags |= 0x001; goto rflag;
            case '*':
                width = (__builtin_va_arg(ap, int));
                if(width >= 0) goto rflag;
                width = -width;
            case '-': flags |= 0x004; goto rflag;
            ...
            case '9':
                ...
                width = n;
                goto reswitch;
            case 'L': flags |= 0x008; goto rflag;
            case 'h': flags |= 0x040; goto rflag;
            case 'l': flags |= 0x010; goto rflag;
            case 'q': flags |= 0x010; goto rflag;
            case 'c':
                cp = buf;
                {
            ...
            case 'G':
                ...
                if(prec == -1) { prec = 6; }
                else if((ch == 'g' || ch == 'G') && prec == 0) { prec = 1; }
                flags |= 0x100;
                ...
                if(ch <= 'e')
                {
                    ...
                    size    = expsize + ndig;
                    if(ndig > 1 || flags & 0x001) size += decp_len;
                }
                else
                {
                    if(ch == 'f')
                    {
                        ...
                        {
                            size = expt;
                            if(prec || flags & 0x001) size += prec + decp_len;
                        }
                        else
                            size = (prec || flags & 0x001) ? prec + 1 + decp_len : 1;
                    }
                    else if(expt >= ndig)
                    {
                        size = expt;
                        if(flags & 0x001) size += decp_len;
                    }
                    else
                    {
                        size = ndig + decp_len;
                        if(expt <= 0) size += 1 - expt;
                    }
                    lead = expt;
                }
                if(softsign) sign = '-';
            ...
            else
            {
                if(ch >= 'f')
                {
                    if((_double_.fp) == 0)
                    {
                        ...
                        {
                            {
                                iovp->iov_base = (decimal_point);
                                iovp->iov_len  = (decp_len);
                                uio.uio_resid += (decp_len);
                                iovp++;
                                if(++uio.uio_iovcnt >= 8)
                                {
                                    if(__sprint_r(data, fp, &uio)) goto error;
                ...
                else
                {
                    ...
                    {
                        iovp->iov_base = (expstr);
                        iovp->iov_len  = (expsize);
...
static char* cvt(struct _reent* data, double value, int ndigits, int flags, char* sign, int* decpt, int ch, int* length, char* buf)
{
    int                mode, dsgn;
    char *             digits, *bp, *rve;
    ...
    if((ch != 'g' && ch != 'G') || flags & 0x001)
    {
        ...
        if(ch == 'f' || ch == 'F')
        {
            if(*digits == '0' && value) *decpt = -ndigits + 1;
            bp += *decpt;
        }
        if(value == 0) rve = bp;
        while(rve < bp) *rve++ = '0';
