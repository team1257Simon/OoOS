...
FILE*                   _fopen_r(struct _reent* ptr, const char* restrict file, const char* restrict mode)
{
    register FILE* fp;
    register int   f;
    int            flags, oflags;
    if((flags = __sflags(ptr, mode, &oflags)) == 0) return ((void*)0);
    ...
    }
    {
        ...
        fp->_seek   = __sseek;
        fp->_close  = __sclose;
        if(fp->_flags & 0x0100) _fseek_r(ptr, fp, 0, 2);
        if(!(fp->_flags2 & 0x0001)) (((fp)->_flags & 0x0200) ? 0 : ((void)0));
    };
    return fp;
}
FILE* fopen(const char* file, const char* mode) { return _fopen_r(_impure_ptr, file, mode); }
