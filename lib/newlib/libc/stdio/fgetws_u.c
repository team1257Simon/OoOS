...
wchar_t*                _fgetws_unlocked_r(struct _reent* ptr, wchar_t* ws, int n, FILE* fp)
{
    wchar_t*       wsp;
    size_t         nconv;
    const char*    src;
    ...
    if(fp->_r <= 0 && __srefill_r(ptr, fp)) goto error;
    wsp = ws;
    do {
        src   = (char*)fp->_p;
        nl    = memchr(fp->_p, '\n', fp->_r);
        nconv = _mbsnrtowcs_r(ptr, wsp, &src, nl != ((void*)0) ? (nl - fp->_p + 1) : fp->_r, n - 1, &fp->_mbstate);
        if(nconv == (size_t)-1) goto error;
        if(src == ((void*)0))
        {
            nconv++;
    ...
    if(wsp == ws) goto error;
    if(!mbsinit(&fp->_mbstate)) goto error;
    *wsp++ = L'\0';
    ;
    return ws;
error:;
...
wchar_t* fgetws_unlocked(wchar_t* restrict ws, int n, FILE* restrict fp)
{
    ...
    return _fgetws_unlocked_r(reent, ws, n, fp);
}
