...
static void             std(FILE* ptr, int flags, int file)
{
    ptr->_p        = 0;
    ptr->_r        = 0;
    ptr->_w        = 0;
...
static inline void stdin_init(FILE* ptr) { std(ptr, 0x0004, 0); }
static inline void stdout_init(FILE* ptr) { std(ptr, 0x0008 | 0x0001, 1); }
static inline void stderr_init(FILE* ptr) { std(ptr, 0x0010 | 0x0002, 2); }
...
FILE* __sfp(struct _reent* d)
{
    ...
    {
        __sfp_lock_acquire();
        if(!_global_impure_ptr->__sdidinit) __sinit(_global_impure_ptr);
        for(g = &_global_impure_ptr->__sglue;; g = g->_next)
        {
            for(fp = g->_iobs, n = g->_niobs; --n >= 0; fp++)
                if(fp->_flags == 0) goto found;
            if(g->_next == ((void*)0) && (g->_next = __sfmoreglue(d, 4)) == ((void*)0)) break;
        }
        __sfp_lock_release();
        ;
        d->_errno = 12;
...
void _cleanup(void) { _cleanup_r(_global_impure_ptr); }
...
static int __sfp_recursive_mutex = 0;
;
static int __sinit_recursive_mutex = 0;
;
void       __sfp_lock_acquire(void) { ((void)0); }
void       __sfp_lock_release(void) { ((void)0); }
void       __sinit_lock_acquire(void) { ((void)0); }
void       __sinit_lock_release(void) { ((void)0); }
static int __fp_lock(FILE* ptr)
{
    if(!(ptr->_flags2 & 0x0001)) (((ptr)->_flags & 0x0200) ? 0 : ((void)0));
    return 0;
}
