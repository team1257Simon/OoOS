...
int                     _fputws_unlocked_r(struct _reent* ptr, const wchar_t* ws, FILE* fp)
{
    size_t        nbytes;
    char          buf[1024];
    struct __suio uio;
    ...
    if(((((fp)->_flags & 0x0008) == 0 || (fp)->_bf._base == ((void*)0)) && __swsetup_r(ptr, fp)) != 0) goto error;
    uio.uio_iov    = &iov;
    uio.uio_iovcnt = 1;
    iov.iov_base   = buf;
    do {
...
int fputws_unlocked(const wchar_t* restrict ws, FILE* restrict fp)
{
    ...
    return _fputws_unlocked_r(reent, ws, fp);
}
