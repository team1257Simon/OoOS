...
int        __sfvwrite_r(struct _reent* ptr, register FILE* fp, register struct __suio* uio)
{
    ...
    if(((((fp)->_flags & 0x0008) == 0 || (fp)->_bf._base == ((void*)0)) && __swsetup_r(ptr, fp))) return (-1);
    iov = uio->uio_iov;
    len = 0;
    if(fp->_flags & 0x0002)
    {
        do {
            ...
            w = fp->_write(ptr, fp->_cookie, p, ((len) < (0x7fffffff - 0x7fffffff % 1024) ? (len) : (0x7fffffff - 0x7fffffff % 1024)));
            if(w <= 0) goto err;
            p += w;
            len -= w;
    ...
    else if((fp->_flags & 0x0001) == 0)
    {
        do {
            ...
            {
                w = ((int)((len) < (0x7fffffff) ? (len) : (0x7fffffff))) / fp->_bf._size * fp->_bf._size;
