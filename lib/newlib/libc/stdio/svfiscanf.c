...
int                                           _sungetc_r(struct _reent* data, int c, register FILE* fp)
{
    if(c == (-1)) return ((-1));
    fp->_flags &= ~0x0020;
    c = (unsigned char)c;
...
int __ssrefill_r(struct _reent* ptr, register FILE* fp)
{
    if(((fp)->_ub._base != ((void*)0)))
    {
        {
...
size_t _sfread_r(struct _reent* ptr, void* buf, size_t size, size_t count, FILE* fp)
{
    ...
    while(resid > (r = fp->_r))
    {
        ...
        p += r;
        resid -= r;
        if(__ssrefill_r(ptr, fp)) { return (total - resid) / size; }
    }
