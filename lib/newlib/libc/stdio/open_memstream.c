...
static int memcloser(struct _reent* ptr, void* cookie)
{
    memstream* c = (memstream*)cookie;
    char*      buf;
    buf = _realloc_r(ptr, *c->pbuf, c->wide > 0 ? (*c->psize + 1) * sizeof(wchar_t) : *c->psize + 1);
    if(buf) *c->pbuf = buf;
    _free_r(ptr, c->storage);
    return 0;
}
static FILE* internal_open_memstream_r(struct _reent* ptr, char** buf, size_t* size, int wide)
{
    FILE*      fp;
    memstream* c;
    if(!buf || !size)
...
FILE* _open_memstream_r(struct _reent* ptr, char** buf, size_t* size) { return internal_open_memstream_r(ptr, buf, size, -1); }
FILE* _open_wmemstream_r(struct _reent* ptr, wchar_t** buf, size_t* size) { return internal_open_memstream_r(ptr, (char**)buf, size, 1); }
FILE* open_memstream(char** buf, size_t* size) { return _open_memstream_r(_impure_ptr, buf, size); }
FILE* open_wmemstream(wchar_t** buf, size_t* size) { return _open_wmemstream_r(_impure_ptr, buf, size); }
