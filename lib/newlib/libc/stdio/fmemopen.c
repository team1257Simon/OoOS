...
FILE* _fmemopen_r(struct _reent* ptr, void* restrict buf, size_t size, const char* restrict mode)
{
    FILE*       fp;
    fmemcookie* c;
    int         flags;
    ...
    if((c = (fmemcookie*)_malloc_r(ptr, sizeof *c + (buf ? 0 : size))) == ((void*)0))
    {
        {
            __sfp_lock_acquire();
            fp->_flags = 0;
    ...
    else
    {
        ...
        case 'a':
            ...
            if(!buf && c->writeonly) c->buf[size - 1] = '\0';
            c->append = 1;
            break;
        case 'r':
            c->pos = c->append = 0;
...
FILE* fmemopen(void* restrict buf, size_t size, const char* restrict mode) { return _fmemopen_r(_impure_ptr, buf, size, mode); }
