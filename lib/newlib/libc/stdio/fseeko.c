...
int                     _fseeko_r(struct _reent* ptr, register FILE* fp, _off_t offset, int whence)
{
    ...
    } while(0);
    {
        ...
        if(fp->_flags & 0x0100 && fp->_flags & 0x0008) { _fflush_r(ptr, fp); }
        ...
        case 1:
            ...
            {
                curoff -= fp->_r;
                if(((fp)->_ub._base != ((void*)0))) curoff -= fp->_ur;
            }
        ...
        if(fp->_bf._base == ((void*)0)) __smakebuf_r(ptr, fp);
        if(fp->_flags & (0x0008 | 0x0010 | 0x0002 | 0x0800)) goto dumb;
        if((fp->_flags & 0x0400) == 0)
        {
            if(seekfn != __sseek || fp->_file < 0 || _fstat_r(ptr, fp->_file, &st) || (st.st_mode & 0170000) != 0100000)
            {
                fp->_flags |= 0x0800;
                goto dumb;
            }
        ...
        if(seekfn(ptr, fp->_cookie, curoff, 0) == (-(_fpos_t)1)) goto dumb;
        fp->_r = 0;
        fp->_p = fp->_bf._base;
        if(((fp)->_ub._base != ((void*)0)))
        {
        ...
        return 0;
    dumb:
        if(_fflush_r(ptr, fp) || seekfn(ptr, fp->_cookie, offset, whence) == (-(_fpos_t)1))
        {
            if(!(fp->_flags2 & 0x0001)) (((fp)->_flags & 0x0200) ? 0 : ((void)0));
            ;
            return (-1);
...
int fseeko(register FILE* fp, _off_t offset, int whence) { return _fseeko_r(_impure_ptr, fp, offset, whence); }
