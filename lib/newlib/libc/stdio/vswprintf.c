...
int                     _vswprintf_r(struct _reent* ptr, wchar_t* str, size_t size, const wchar_t* fmt, va_list ap)
{
    int  ret;
    FILE f;
    if(size > 0x7fffffff / sizeof(wchar_t))
    {
        ptr->_errno = 139;
        return (-1);
    }
...
int vswprintf(wchar_t* restrict str, size_t size, const wchar_t* restrict fmt, va_list ap) { return _vswprintf_r(_impure_ptr, str, size, fmt, ap); }
