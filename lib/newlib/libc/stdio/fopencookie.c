...
static int fcreader(struct _reent* ptr, void* cookie, char* buf, int n)
{
    ...
    (*__errno()) = 0;
    if((result = c->readfn(c->cookie, buf, n)) < 0 && (*__errno())) ptr->_errno = (*__errno());
    return result;
}
static int fcwriter(struct _reent* ptr, void* cookie, const char* buf, int n)
{
    int       result;
    fccookie* c = (fccookie*)cookie;
    if(c->fp->_flags & 0x0100 && c->fp->_seek) { c->fp->_seek(ptr, cookie, 0, 2); }
    (*__errno()) = 0;
    if((result = c->writefn(c->cookie, buf, n)) < 0 && (*__errno())) ptr->_errno = (*__errno());
    return result;
}
static _fpos_t fcseeker(struct _reent* ptr, void* cookie, _fpos_t pos, int whence)
{
    ...
    (*__errno())     = 0;
    if(c->seekfn(c->cookie, &offset, whence) < 0 && (*__errno())) ptr->_errno = (*__errno());
...
static int fccloser(struct _reent* ptr, void* cookie)
{
    ...
    {
        (*__errno()) = 0;
        if((result = c->closefn(c->cookie)) < 0 && (*__errno())) ptr->_errno = (*__errno());
    }
    _free_r(ptr, c);
    return result;
}
FILE* _fopencookie_r(struct _reent* ptr, void* cookie, const char* mode, cookie_io_functions_t functions)
{
    ...
    int       flags;
    int       dummy;
    if((flags = __sflags(ptr, mode, &dummy)) == 0) return ((void*)0);
    if(((flags & (0x0004 | 0x0010)) && !functions.read) || ((flags & (0x0008 | 0x0010)) && !functions.write))
    {
        ptr->_errno = 22;
        return ((void*)0);
    }
    if((fp = __sfp(ptr)) == ((void*)0)) return ((void*)0);
...
FILE* fopencookie(void* cookie, const char* mode, cookie_io_functions_t functions) { return _fopencookie_r(_impure_ptr, cookie, mode, functions); }
