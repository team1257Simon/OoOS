...
int                vfwscanf(register FILE* restrict fp, const wchar_t* restrict fmt, va_list ap)
{
    struct _reent* reent = _impure_ptr;
    do {
        struct _reent* _check_init_ptr = (reent);
        if((_check_init_ptr) && !(_check_init_ptr)->__sdidinit) __sinit(_check_init_ptr);
    } while(0);
    return __svfwscanf_r(reent, fp, fmt, ap);
}
int __svfwscanf(register FILE* fp, wchar_t const* fmt0, va_list ap) { return __svfwscanf_r(_impure_ptr, fp, fmt0, ap); }
...
int __svfwscanf_r(struct _reent* rptr, register FILE* fp, wchar_t const* fmt0, va_list ap)
{
    ...
    decpt                          = (wchar_t)*_localeconv_r(rptr)->decimal_point;
    {
        ...
        for(;;)
        {
            ...
            if(iswspace(c))
            {
                while((c = _fgetwc_r(rptr, fp)) != ((wint_t)-1) && iswspace(c))
                    ;
                if(c != ((wint_t)-1)) _ungetwc_r(rptr, c, fp);
                continue;
            }
            if(c != L'%') goto literal;
            width = 0;
        ...
        again:
            ...
            case L'%':
            literal:
                if((wi = _fgetwc_r(rptr, fp)) == ((wint_t)-1)) goto input_failure;
                if(wi != c)
                {
                ...
                continue;
            case L'*':
                if((flags & (0x08 | 0x04 | 0x01 | 0x02 | 0x10 | 0x80)) || width) flags |= 0x10;
                goto again;
            case L'l':
                if(flags & (0x08 | 0x04 | 0x01 | 0x02)) goto match_failure;
                flags |= 0x01;
                goto again;
            case L'L':
                if(flags & (0x08 | 0x04 | 0x01 | 0x02)) goto match_failure;
                flags |= 0x02;
                goto again;
            case L'h': flags |= 0x04; goto again;
            case L'0':
            case L'1':
            case L'2':
            case L'3':
            ...
            case L'8':
            case L'9':
                if(flags & (0x08 | 0x04 | 0x01 | 0x02 | 0x80)) goto match_failure;
            ...
            if((flags & 0x40) == 0)
            {
                while((wi = _fgetwc_r(rptr, fp)) != ((wint_t)-1) && iswspace(wi)) nread++;
                if(wi == ((wint_t)-1)) goto input_failure;
                _ungetwc_r(rptr, wi, fp);
            }
            switch(c)
            {
            case 0:
                ...
                if(flags & 0x01)
                {
                    ...
                    {
                        if(!(flags & 0x10)) { *p++ = (wchar_t)wi; }
                        n++;
                    }
                    if(n == 0) goto input_failure;
                    nread += n;
                    if(!(flags & 0x10)) nassigned++;
                }
                else
                {
                    if(flags & 0x10)
                ...
                break;
            case 1:
                if(width == 0) width = (0xffffffffffffffffUL);
                if((flags & 0x10) && (flags & 0x01))
                {
                    n = 0;
                    while((wi = _fgetwc_r(rptr, fp)) != ((wint_t)-1) && width-- != 0 &&
                          (cclcompl ? (wmemchr(ccls, (wi), ccle - ccls) == ((void*)0)) : (wmemchr(ccls, (wi), ccle - ccls) != ((void*)0))))
                        n++;
                    if(wi != ((wint_t)-1)) _ungetwc_r(rptr, wi, fp);
            ...
            case 2:
                ...
                if((flags & 0x10) && (flags & 0x01))
                {
                    while((wi = _fgetwc_r(rptr, fp)) != ((wint_t)-1) && width-- != 0 && !iswspace(wi)) nread++;
                    if(wi != ((wint_t)-1)) _ungetwc_r(rptr, wi, fp);
                }
                else if(flags & 0x01)
                {
                    p0 = p = (__builtin_va_arg(ap, wchar_t*));
                    while((wi = _fgetwc_r(rptr, fp)) != ((wint_t)-1) && width-- != 0 && !iswspace(wi))
                    {
                        *p++ = (wchar_t)wi;
                        nread++;
                    }
            ...
            case 3:
            {
                if(width == 0 || width > sizeof(buf) / sizeof(*buf) - 1) width = sizeof(buf) / sizeof(*buf) - 1;
            ...
            case 4:
            {
                ...
                for(p = buf; width;)
                {
                    ...
                    case L'n':
                    case L'N':
                        if(nancount == 0 && zeroes == 0 && (flags & (0x100 | 0x200 | 0x400)) == (0x100 | 0x200 | 0x400))
                        {
                    ...
                    case L'A':
                        ...
                        break;
                    case L'i':
                        if(infcount == 0 && zeroes == 0 && (flags & (0x100 | 0x200 | 0x400)) == (0x100 | 0x200 | 0x400))
                        {
                            flags &= ~(0x80 | 0x200 | 0x400 | 0x100);
                            infcount = 1;
                            goto fok;
                    ...
                    case L'e':
                    case L'E':
                        if((flags & (0x100 | 0x400)) == 0x400 || ((flags & 0x400) && zeroes))
                        {
                            if(!(flags & 0x200))
                            {
                            ...
                            flags  = (flags & ~(0x400 | 0x200)) | 0x80 | 0x100;
                            zeroes = 0;
                            goto fok;
                        }
                        break;
                ...
                if((flags & 0x10) == 0)
                {
                    ...
                    *p             = 0;
                    ...
                    if(exp_adjust)
                    {
                        if(exp_start >= buf + sizeof(buf) / sizeof(*buf) - ((8 * sizeof(long) - 1) * 4 / 13 + 2))
                            exp_start = buf + sizeof(buf) / sizeof(*buf) - ((8 * sizeof(long) - 1) * 4 / 13 + 2) - 1;
