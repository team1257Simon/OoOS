...
int vsnprintf(char* restrict str, size_t size, const char* restrict fmt, va_list ap) { return _vsnprintf_r(_impure_ptr, str, size, fmt, ap); }
int _vsnprintf_r(struct _reent* ptr, char* restrict str, size_t size, const char* restrict fmt, va_list ap)
{
    int  ret;
    FILE f;
    if(size > 0x7fffffff)
