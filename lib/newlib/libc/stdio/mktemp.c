...
static int        _gettemp(struct _reent* ptr, char* path, register int* doopen, int domkdir, size_t suffixlen, int flags)
{
    ...
    for(;;)
    {
        ...
        {
            if((*doopen = _open_r(ptr, path, 0x0200 | 0x0800 | 2 | flags, 0600)) >= 0) return 1;
            if(ptr->_errno != 17) return 0;
        }
        ...
        for(trv = start;;)
        {
            ...
            {
                if(((((_ctype_) + sizeof(""[(unsigned char)*trv]))[(int)((unsigned char)*trv)]) & 04))
                    *trv = 'a';
                else
                    ++*trv;
                break;
...
{
    int fd;
    return (_gettemp(ptr, path, &fd, 0, 0, 0) ? fd : -1);
}
char* _mkdtemp_r(struct _reent* ptr, char* path) { return (_gettemp(ptr, path, (int*)((void*)0), 1, 0, 0) ? path : ((void*)0)); }
int   _mkstemps_r(struct _reent* ptr, char* path, int len)
{
    int fd;
    return (_gettemp(ptr, path, &fd, 0, len, 0) ? fd : -1);
}
...
char* _mktemp_r(struct _reent* ptr, char* path) { return (_gettemp(ptr, path, (int*)((void*)0), 0, 0, 0) ? path : (char*)((void*)0)); }
int   mkstemp(char* path)
{
    int fd;
    return (_gettemp(_impure_ptr, path, &fd, 0, 0, 0) ? fd : -1);
}
char* mkdtemp(char* path) { return (_gettemp(_impure_ptr, path, (int*)((void*)0), 1, 0, 0) ? path : ((void*)0)); }
int   mkstemps(char* path, int len)
{
    int fd;
    return (_gettemp(_impure_ptr, path, &fd, 0, len, 0) ? fd : -1);
}
...
char* mktemp(char* path) { return (_gettemp(_impure_ptr, path, (int*)((void*)0), 0, 0, 0) ? path : (char*)((void*)0)); }
