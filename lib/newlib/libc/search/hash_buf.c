...
extern BUFHEAD* __get_buf(hashp, addr, prev_bp, newpage)
HTAB*           hashp;
__uint32_t      addr;
BUFHEAD*        prev_bp;
int             newpage;
{
    ...
    {
        bp = prev_bp->ovfl;
        if(!bp || (bp->addr != addr)) bp = ((void*)0);
        if(!newpage) is_disk = 0x0002;
    }
    else
    {
    ...
    if(!bp)
    {
        ...
        if(!bp || __get_page(hashp, bp->page, addr, !prev_bp, is_disk, 0)) return (((void*)0));
        if(!prev_bp) segp[segment_ndx] = (BUFHEAD*)((ptrdiff_t)bp | (intptr_t)is_disk_mask);
    }
    else
    {
        {
...
static BUFHEAD* newbuf(hashp, addr, prev_bp)
HTAB*           hashp;
__uint32_t      addr;
BUFHEAD*        prev_bp;
{
    ...
    {
        if((bp = (BUFHEAD*)malloc(sizeof(BUFHEAD))) == ((void*)0)) return (((void*)0));
    ...
    else
    {
        ...
        if((bp->addr != 0) || (bp->flags & 0x0004))
        {
            ...
            if((bp->flags & 0x0001) && __put_page(hashp, bp->page, bp->addr, (int)((bp->flags) & 0x0004), 0)) return (((void*)0));
            if(((bp->flags) & 0x0004))
            {
                segment_ndx = bp->addr & (hashp->hdr.ssize - 1);
                segp        = hashp->dir[bp->addr >> hashp->hdr.sshift];
                if(hashp->new_file && ((bp->flags & 0x0001) || ((__uint32_t)(ptrdiff_t)(segp[segment_ndx]) & 0x2)))
            ...
            for(xbp = bp; xbp->ovfl;)
            {
                ...
                xbp->ovfl = 0;
                xbp       = next_xbp;
                if(((xbp->flags) & 0x0004) || (oaddr != xbp->addr)) break;
                shortp = (__uint16_t*)xbp->page;
                if(shortp[0]) oaddr = shortp[shortp[0] - 1];
                if((xbp->flags & 0x0001) && __put_page(hashp, xbp->page, xbp->addr, 0, 0)) return (((void*)0));
                xbp->addr  = 0;
                xbp->flags = 0;
                {
...
extern int __buf_free(hashp, do_free, to_disk)
HTAB*      hashp;
int        do_free, to_disk;
{
    BUFHEAD* bp;
    if(!hashp->bufhead.prev) return (0);
    for(bp = hashp->bufhead.prev; bp != &hashp->bufhead;)
    {
        if(bp->addr || ((bp->flags) & 0x0004))
        {
            if(to_disk && (bp->flags & 0x0001) && __put_page(hashp, bp->page, bp->addr, ((bp->flags) & 0x0004), 0)) return (-1);
        }
        if(do_free)
        {
