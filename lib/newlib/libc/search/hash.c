...
extern DB*   __hash_open(const char* file, int flags, int mode, int dflags, const HASHINFO* info)
{
    HTAB*       hashp;
    struct stat statbuf;
    DB*         dbp;
    ...
    if(!(hashp = (HTAB*)calloc(1, sizeof(HTAB)))) return (((void*)0));
    hashp->fp    = -1;
    hashp->flags = flags;
    new_table    = 0;
    if(!file || (flags & 0x0400) || (stat(file, &statbuf) && ((*__errno()) == 2)))
    {
        if((*__errno()) == 2) (*__errno()) = 0;
        new_table = 1;
    }
    if(file)
    {
        ...
        if((flags & 0x0200) && fstat(hashp->fp, &statbuf) == 0 && statbuf.st_size == 0) new_table = 1;
    }
    if(new_table)
    {
    ...
    else
    {
        ...
        if(hashp->hdr.version != 2 && hashp->hdr.version != 1)
        {
            save_errno = 79;
            goto error1;
        };
        ...
        hashp->nsegs = 0;
        if(alloc_segs(hashp, nsegs)) return (((void*)0));
    ...
    hashp->save_file = file && (hashp->flags & 2);
    hashp->cbucket   = -1;
    ...
    return (dbp);
error1:
    if(hashp != ((void*)0)) (void)close(hashp->fp);
error0:
    free(hashp);
    (*__errno()) = save_errno;
    return (((void*)0));
}
static int hash_close(dbp)
DB*        dbp;
{
    HTAB* hashp;
    int   retval;
...
static HTAB*    init_hash(hashp, file, info)
HTAB*           hashp;
const char*     file;
const HASHINFO* info;
{
    ...
    if(file != ((void*)0))
    {
        if(stat(file, &statbuf)) return (((void*)0));
        hashp->hdr.bsize  = ((statbuf.st_blksize) < (65536) ? (statbuf.st_blksize) : (65536));
    ...
    if(info)
    {
        ...
        if(info->ffactor) hashp->hdr.ffactor = info->ffactor;
        if(info->hash) hashp->hash = info->hash;
        if(info->nelem) nelem = info->nelem;
        if(info->lorder)
        {
            if(info->lorder != 4321 && info->lorder != 1234)
            {
                (*__errno()) = 22;
                return (((void*)0));
            }
...
static int init_htab(hashp, nelem)
HTAB*      hashp;
int        nelem;
{
    int nbuckets, nsegs;
    ...
    l2                        = __log2(((nelem) > (2) ? (nelem) : (2)));
    nbuckets                  = 1 << l2;
    hashp->hdr.spares[l2]     = l2 + 1;
    hashp->hdr.spares[l2 + 1] = l2 + 1;
    hashp->hdr.ovfl_point     = l2;
    hashp->hdr.last_freed     = 2;
    if(__ibitmap(hashp, ((__uint32_t)((__uint32_t)(l2) << 11) + (1)), l2 + 1, 0)) return (-1);
    hashp->hdr.max_bucket = hashp->hdr.low_mask = nbuckets - 1;
    hashp->hdr.high_mask                        = (nbuckets << 1) - 1;
    hashp->hdr.hdrpages                         = ((((sizeof(HASHHDR)) > (512) ? (sizeof(HASHHDR)) : (512)) - 1) >> hashp->hdr.bshift) + 1;
    nsegs                                       = (nbuckets - 1) / hashp->hdr.ssize + 1;
    nsegs                                       = 1 << __log2(nsegs);
    if(nsegs > hashp->hdr.dsize) hashp->hdr.dsize = nsegs;
    return (alloc_segs(hashp, nsegs));
}
static int hdestroy(hashp)
HTAB*      hashp;
{
    int i, save_errno;
    save_errno = 0;
    if(__buf_free(hashp, 1, hashp->save_file)) save_errno = (*__errno());
    if(hashp->dir)
    {
        free(*hashp->dir);
    ...
    if(flush_meta(hashp) && !save_errno) save_errno = (*__errno());
...
u_int      flags;
{
    ...
    if(__buf_free(hashp, 0, 1) || flush_meta(hashp)) return ((-1));
    hashp->new_file = 0;
    return (0);
}
static int flush_meta(hashp)
HTAB*      hashp;
{
    HASHHDR* whdrp;
    HASHHDR  whdr;
    ...
    if((lseek(fp, (off_t)0, 0) == -1) || ((wsize = write(fp, whdrp, sizeof(HASHHDR))) == -1))
        return (-1);
    ...
    for(i = 0; i < 32; i++)
        if(hashp->mapp[i])
            if(__put_page(hashp, (char*)hashp->mapp[i], hashp->hdr.bitmaps[i], 0, 1)) return (-1);
    return (0);
}
...
u_int      flag;
{
    ...
    if((hashp->flags & (0 | 1 | 2)) == 0)
    {
        hashp->error = (*__errno()) = 1;
        return ((-1));
    }
    return (hash_access(hashp, flag == 8 ? HASH_PUTNEW : HASH_PUT, (DBT*)key, (DBT*)data));
}
...
static int hash_access(hashp, action, key, val)
HTAB*      hashp;
ACTION     action;
DBT *      key, *val;
{
    ...
    case HASH_GET:
    case HASH_DELETE:
    default: save_bufp->flags &= ~0x0008; return ((1));
    }
found:
    switch(action)
    {
    case HASH_PUTNEW: save_bufp->flags &= ~0x0008; return ((1));
    case HASH_GET:
        bp = (__uint16_t*)rbufp->page;
        if(bp[ndx + 1] < 4)
        {
            if(__big_return(hashp, rbufp, ndx, val, 0)) return ((-1));
        }
        else
        {
    ...
    case HASH_PUT:
        ...
        break;
    case HASH_DELETE:
        if(__delpair(hashp, rbufp, ndx)) return ((-1));
        break;
    default: abort();
    }
    save_bufp->flags &= ~0x0008;
...
u_int      flag;
{
    ...
    for(bp = ((void*)0); !bp || !bp[0];)
    {
        if(!(bufp = hashp->cpage))
        {
            for(bucket = hashp->cbucket; bucket <= hashp->hdr.max_bucket; bucket++, hashp->cndx = 1)
            {
                bufp = __get_buf(hashp, bucket, ((void*)0), 0);
                if(!bufp) return ((-1));
                hashp->cpage = bufp;
    ...
    if(bp[ndx + 1] < 4)
    {
        if(__big_keydata(hashp, bufp, key, data, 1)) return ((-1));
    }
    else
    {
...
extern int __expand_table(hashp)
HTAB*      hashp;
{
    ...
    if(new_segnum >= hashp->nsegs)
    {
        ...
        if((hashp->dir[new_segnum] = (SEGMENT)calloc(hashp->hdr.ssize, sizeof(SEGMENT))) == ((void*)0)) return (-1);
        hashp->exsegs++;
        hashp->nsegs++;
    }
...
static void* hash_realloc(p_ptr, oldsize, newsize)
SEGMENT**    p_ptr;
int          oldsize, newsize;
{
    void* p;
...
extern __uint32_t __call_hash(hashp, k, len)
HTAB*             hashp;
char*             k;
int               len;
{
    ...
    if(bucket > hashp->hdr.max_bucket) bucket = bucket & hashp->hdr.low_mask;
    return (bucket);
}
static int alloc_segs(hashp, nsegs)
HTAB*      hashp;
int        nsegs;
{
    int     i;
    SEGMENT store;
    int     save_errno;
    if((hashp->dir = (SEGMENT*)calloc(hashp->hdr.dsize, sizeof(SEGMENT*))) == ((void*)0))
    {
        save_errno = (*__errno());
        (void)hdestroy(hashp);
        (*__errno()) = save_errno;
    ...
    if((store = (SEGMENT)calloc(nsegs << hashp->hdr.sshift, sizeof(SEGMENT))) == ((void*)0))
    {
        save_errno = (*__errno());
        (void)hdestroy(hashp);
        (*__errno()) = save_errno;
    ...
    for(i = 0; i < nsegs; i++, hashp->nsegs++) hashp->dir[i] = &store[i << hashp->hdr.sshift];
    return (0);
}
...
static void swap_header(hashp) HTAB* hashp;
{
    ...
    {
        __uint32_t _tmp            = hdrp->version;
        ((char*)&hdrp->version)[0] = ((char*)&_tmp)[3];
        ((char*)&hdrp->version)[1] = ((char*)&_tmp)[2];
        ((char*)&hdrp->version)[2] = ((char*)&_tmp)[1];
        ((char*)&hdrp->version)[3] = ((char*)&_tmp)[0];
    };
    {
        __uint32_t _tmp           = hdrp->lorder;
        ((char*)&hdrp->lorder)[0] = ((char*)&_tmp)[3];
        ((char*)&hdrp->lorder)[1] = ((char*)&_tmp)[2];
        ((char*)&hdrp->lorder)[2] = ((char*)&_tmp)[1];
        ((char*)&hdrp->lorder)[3] = ((char*)&_tmp)[0];
    };
    {
        __uint32_t _tmp          = hdrp->bsize;
        ((char*)&hdrp->bsize)[0] = ((char*)&_tmp)[3];
        ((char*)&hdrp->bsize)[1] = ((char*)&_tmp)[2];
        ((char*)&hdrp->bsize)[2] = ((char*)&_tmp)[1];
        ((char*)&hdrp->bsize)[3] = ((char*)&_tmp)[0];
    };
    {
        __uint32_t _tmp           = hdrp->bshift;
        ((char*)&hdrp->bshift)[0] = ((char*)&_tmp)[3];
        ((char*)&hdrp->bshift)[1] = ((char*)&_tmp)[2];
        ((char*)&hdrp->bshift)[2] = ((char*)&_tmp)[1];
        ((char*)&hdrp->bshift)[3] = ((char*)&_tmp)[0];
    };
    {
        __uint32_t _tmp          = hdrp->dsize;
        ((char*)&hdrp->dsize)[0] = ((char*)&_tmp)[3];
        ((char*)&hdrp->dsize)[1] = ((char*)&_tmp)[2];
        ((char*)&hdrp->dsize)[2] = ((char*)&_tmp)[1];
        ((char*)&hdrp->dsize)[3] = ((char*)&_tmp)[0];
    };
    {
        __uint32_t _tmp          = hdrp->ssize;
        ((char*)&hdrp->ssize)[0] = ((char*)&_tmp)[3];
        ((char*)&hdrp->ssize)[1] = ((char*)&_tmp)[2];
        ((char*)&hdrp->ssize)[2] = ((char*)&_tmp)[1];
        ((char*)&hdrp->ssize)[3] = ((char*)&_tmp)[0];
    };
    {
        __uint32_t _tmp           = hdrp->sshift;
        ((char*)&hdrp->sshift)[0] = ((char*)&_tmp)[3];
        ((char*)&hdrp->sshift)[1] = ((char*)&_tmp)[2];
        ((char*)&hdrp->sshift)[2] = ((char*)&_tmp)[1];
        ((char*)&hdrp->sshift)[3] = ((char*)&_tmp)[0];
    };
    {
        __uint32_t _tmp               = hdrp->ovfl_point;
        ((char*)&hdrp->ovfl_point)[0] = ((char*)&_tmp)[3];
        ((char*)&hdrp->ovfl_point)[1] = ((char*)&_tmp)[2];
        ((char*)&hdrp->ovfl_point)[2] = ((char*)&_tmp)[1];
        ((char*)&hdrp->ovfl_point)[3] = ((char*)&_tmp)[0];
    };
    {
        __uint32_t _tmp               = hdrp->last_freed;
        ((char*)&hdrp->last_freed)[0] = ((char*)&_tmp)[3];
        ((char*)&hdrp->last_freed)[1] = ((char*)&_tmp)[2];
        ((char*)&hdrp->last_freed)[2] = ((char*)&_tmp)[1];
        ((char*)&hdrp->last_freed)[3] = ((char*)&_tmp)[0];
    };
    {
        __uint32_t _tmp               = hdrp->max_bucket;
        ((char*)&hdrp->max_bucket)[0] = ((char*)&_tmp)[3];
        ((char*)&hdrp->max_bucket)[1] = ((char*)&_tmp)[2];
        ((char*)&hdrp->max_bucket)[2] = ((char*)&_tmp)[1];
        ((char*)&hdrp->max_bucket)[3] = ((char*)&_tmp)[0];
    };
    {
        __uint32_t _tmp              = hdrp->high_mask;
        ((char*)&hdrp->high_mask)[0] = ((char*)&_tmp)[3];
        ((char*)&hdrp->high_mask)[1] = ((char*)&_tmp)[2];
        ((char*)&hdrp->high_mask)[2] = ((char*)&_tmp)[1];
        ((char*)&hdrp->high_mask)[3] = ((char*)&_tmp)[0];
    };
    {
        __uint32_t _tmp             = hdrp->low_mask;
        ((char*)&hdrp->low_mask)[0] = ((char*)&_tmp)[3];
        ((char*)&hdrp->low_mask)[1] = ((char*)&_tmp)[2];
        ((char*)&hdrp->low_mask)[2] = ((char*)&_tmp)[1];
        ((char*)&hdrp->low_mask)[3] = ((char*)&_tmp)[0];
    };
    {
        __uint32_t _tmp            = hdrp->ffactor;
        ((char*)&hdrp->ffactor)[0] = ((char*)&_tmp)[3];
        ((char*)&hdrp->ffactor)[1] = ((char*)&_tmp)[2];
        ((char*)&hdrp->ffactor)[2] = ((char*)&_tmp)[1];
        ((char*)&hdrp->ffactor)[3] = ((char*)&_tmp)[0];
    };
    {
        __uint32_t _tmp          = hdrp->nkeys;
        ((char*)&hdrp->nkeys)[0] = ((char*)&_tmp)[3];
        ((char*)&hdrp->nkeys)[1] = ((char*)&_tmp)[2];
        ((char*)&hdrp->nkeys)[2] = ((char*)&_tmp)[1];
        ((char*)&hdrp->nkeys)[3] = ((char*)&_tmp)[0];
    };
    {
        __uint32_t _tmp             = hdrp->hdrpages;
        ((char*)&hdrp->hdrpages)[0] = ((char*)&_tmp)[3];
        ((char*)&hdrp->hdrpages)[1] = ((char*)&_tmp)[2];
        ((char*)&hdrp->hdrpages)[2] = ((char*)&_tmp)[1];
        ((char*)&hdrp->hdrpages)[3] = ((char*)&_tmp)[0];
    };
    {
        __uint32_t _tmp              = hdrp->h_charkey;
        ((char*)&hdrp->h_charkey)[0] = ((char*)&_tmp)[3];
        ((char*)&hdrp->h_charkey)[1] = ((char*)&_tmp)[2];
        ((char*)&hdrp->h_charkey)[2] = ((char*)&_tmp)[1];
        ((char*)&hdrp->h_charkey)[3] = ((char*)&_tmp)[0];
    };
    for(i = 0; i < 32; i++)
    {
        {
            __uint32_t _tmp              = hdrp->spares[i];
            ((char*)&hdrp->spares[i])[0] = ((char*)&_tmp)[3];
            ((char*)&hdrp->spares[i])[1] = ((char*)&_tmp)[2];
            ((char*)&hdrp->spares[i])[2] = ((char*)&_tmp)[1];
            ((char*)&hdrp->spares[i])[3] = ((char*)&_tmp)[0];
        };
        {
            __uint16_t _tmp               = hdrp->bitmaps[i];
            ((char*)&hdrp->bitmaps[i])[0] = ((char*)&_tmp)[1];
            ((char*)&hdrp->bitmaps[i])[1] = ((char*)&_tmp)[0];
        };
    }
}
