...
struct internal_entry
{
    struct
    {
        struct internal_entry* sle_next;
    } link;
    ENTRY ent;
};
struct internal_head
{
    struct internal_entry* slh_first;
};
...
int hcreate_r(size_t nel, struct hsearch_data* htab)
{
    ...
    if(nel < (1 << 4)) nel = (1 << 4);
    if(nel > ((size_t)1 << (sizeof(size_t) * 8 - 1 - 5))) nel = ((size_t)1 << (sizeof(size_t) * 8 - 1 - 5));
    if((nel & (nel - 1)) != 0)
    {
    ...
    for(idx = 0; idx < htab->htablesize; idx++) do
        {
            (((&(htab->htable[idx])))->slh_first) = ((void*)0);
        } while(0);
    return 1;
}
void hdestroy_r(struct hsearch_data* htab)
{
    if(htab->htable == ((void*)0)) return;
    free(htab->htable);
    htab->htable = ((void*)0);
}
int hsearch_r(ENTRY item, ACTION action, ENTRY** retval, struct hsearch_data* htab)
{
    ...
    do {
        (((ie))->link.sle_next) = (((head))->slh_first);
        (((head))->slh_first)   = (ie);
    } while(0);
    *retval = &ie->ent;
    return 1;
