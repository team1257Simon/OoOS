...
static void        trecurse(root, action, level) const node_t* root;
...
int level;
{
    ...
    else
    {
        ...
        if(root->llink != ((void*)0)) trecurse(root->llink, action, level + 1);
        (*action)(root, postorder, level);
        if(root->rlink != ((void*)0)) trecurse(root->rlink, action, level + 1);
        (*action)(root, endorder, level);
    }
}
void twalk(const void* vroot, void (*action)(const void*, VISIT, int))
{
    if(vroot != ((void*)0) && action != ((void*)0)) trecurse(vroot, action, 0);
}
