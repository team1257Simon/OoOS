...
char*                      __strncpy_chk(char* restrict, const char* restrict, size_t, size_t);
char*                      __strncpy_chk(char* restrict dst, const char* restrict src, size_t len, size_t slen)
{
    if(len > slen) __chk_fail();
    if((((src) <= (dst) && (dst) < (src) + (len)) || ((dst) <= (src) && (src) < (dst) + (len)))) __chk_fail();
