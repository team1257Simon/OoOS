...
char*        __gets_chk(char* restrict buf, size_t slen)
{
    char*  abuf;
    size_t len;
    if(slen >= (size_t)0x7fffffff) return gets(buf);
    if((abuf = malloc(slen + 1)) == ((void*)0)) return gets(buf);
    if(fgets(abuf, (int)(slen + 1), (_impure_ptr->_stdin)) == ((void*)0))
    {
        free(abuf);
        return ((void*)0);
    }
