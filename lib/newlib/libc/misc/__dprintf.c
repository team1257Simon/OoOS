    ...
    while(*fmt)
    {
        ...
        if(*fmt == 'N')
        {
            ...
            while(--count >= 0)
            {
                switch(c)
                {
                case 'c': write_string(__unctrl[(*p++) & 0xff]); break;
                case 'p':
                ...
                case 'u':
                case 'x':
                    print_number(c == 'x' ? 16 : 10, c != 'd', get_number(p, sizeof(int), c != 'd'));
                    p += sizeof(int);
                    break;
                case 's':
                    write_string(*(char**)p);
...
static void write_char(char c) { _write_r(_impure_ptr, 2, &c, 1); }
static void write_string(const char* s) { _write_r(_impure_ptr, 2, s, strlen(s)); }
