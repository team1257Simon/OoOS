...
char* _setlocale_r(struct _reent* p, int category, const char* locale)
{
    if(locale)
    {
        if(strcmp(locale, "POSIX") && strcmp(locale, "C") && strcmp(locale, "")) return ((void*)0);
    }
    return "C";
}
int   __locale_mb_cur_max(void) { return __get_current_locale()->mb_cur_max[0]; }
char* setlocale(int category, const char* locale) { return _setlocale_r(_impure_ptr, category, locale); }
