...
struct __locale_t*                            _uselocale_r(struct _reent* p, struct __locale_t* newloc)
{
    ...
    if(!current_locale) current_locale = ((struct __locale_t*)-1);
    if(newloc == ((struct __locale_t*)-1))
        p->_locale = ((void*)0);
    else if(newloc)
        p->_locale = newloc;
    return current_locale;
}
struct __locale_t* uselocale(struct __locale_t* newloc) { return _uselocale_r(_impure_ptr, newloc); }
