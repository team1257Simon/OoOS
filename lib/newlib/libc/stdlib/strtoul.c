...
static unsigned long _strtoul_l(struct _reent* rptr, const char* restrict nptr, char** restrict endptr, int base, locale_t loc)
{
    ...
    cutoff = (unsigned long)(0x7fffffffffffffffL * 2UL + 1UL) / (unsigned long)base;
    cutlim = (unsigned long)(0x7fffffffffffffffL * 2UL + 1UL) % (unsigned long)base;
    for(acc = 0, any = 0;; c = *s++)
    {
        if(c >= '0' && c <= '9')
            c -= '0';
    ...
    if(any < 0)
    {
        acc          = (0x7fffffffffffffffL * 2UL + 1UL);
        rptr->_errno = 34;
    }
    else if(neg)
        acc = -acc;
...
unsigned long _strtoul_r(struct _reent* rptr, const char* restrict nptr, char** restrict endptr, int base)
{
    return _strtoul_l(rptr, nptr, endptr, base, __get_current_locale());
}
unsigned long strtoul_l(const char* restrict s, char** restrict ptr, int base, locale_t loc) { return _strtoul_l(_impure_ptr, s, ptr, base, loc); }
unsigned long strtoul(const char* restrict s, char** restrict ptr, int base) { return _strtoul_l(_impure_ptr, s, ptr, base, __get_current_locale()); }
