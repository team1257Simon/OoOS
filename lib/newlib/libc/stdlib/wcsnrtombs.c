...
size_t _wcsnrtombs_l(struct _reent* r, char* dst, const wchar_t** src, size_t nwc, size_t len, mbstate_t* ps, struct __locale_t* loc)
{
    char*    ptr = dst;
    char     buff[10];
    wchar_t* pwcs;
...
size_t _wcsnrtombs_r(struct _reent* r, char* dst, const wchar_t** src, size_t nwc, size_t len, mbstate_t* ps)
{
    return _wcsnrtombs_l(_impure_ptr, dst, src, nwc, len, ps, __get_current_locale());
}
