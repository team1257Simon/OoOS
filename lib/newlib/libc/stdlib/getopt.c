...
int          __getopt_long_r(int __argc, char* const __argv[], const char* __shortopts, const struct option* __longopts, int* __longind,
                             struct getopt_data* __data);
int          __getopt_long_only_r(int __argc, char* const __argv[], const char* __shortopts, const struct option* __longopts, int* __longind,
                                  struct getopt_data* __data);
...
static int  is_option(char* argv_element, int only) { return ((argv_element == 0) || (argv_element[0] == '-') || (only && argv_element[0] == '+')); }
                           ...
                           struct getopt_data* data)
{
    ...
    int               arg_next      = 0;
    int               initial_colon = 0;
    if(argc == 0 || argv == 0 || (shortopts == 0 && longopts == 0) || data->optind >= argc || argv[data->optind] == 0) return (-1);
    ...
    if(data->optwhere == 1)
    {
        ...
        case RETURN_IN_ORDER:
            ...
            break;
        case REQUIRE_ORDER:
            if(!is_option(argv[data->optind], only)) return (-1);
            break;
        }
    }
    if(argv[data->optind] == 0) return (-1);
    if(longopts != 0 && (memcmp(argv[data->optind], "--", 2) == 0 || (only && argv[data->optind][0] == '+')) && data->optwhere == 1)
    {
        ...
        {
            if(memcmp(argv[data->optind] + data->optwhere, longopts[optindex].name, match_chars) == 0)
            {
...
int __getopt_r(int argc, char* const argv[], const char* optstring, struct getopt_data* data)
{
    return getopt_internal(argc, argv, optstring, 0, 0, 0, data);
}
int __getopt_long_r(int argc, char* const argv[], const char* shortopts, const struct option* longopts, int* longind, struct getopt_data* data)
{
    return getopt_internal(argc, argv, shortopts, longopts, longind, 0, data);
}
int __getopt_long_only_r(int argc, char* const argv[], const char* shortopts, const struct option* longopts, int* longind, struct getopt_data* data)
{
