...
static intmax_t _strtoimax_l(struct _reent* rptr, const char* restrict nptr, char** restrict endptr, int base, locale_t loc)
{
    const char* s   = nptr;
    uintmax_t   acc = 0;
    char        c;
    ...
    cutoff = neg ? -(uintmax_t)(-(0x7fffffffffffffffL) - 1) : (0x7fffffffffffffffL);
    cutlim = cutoff % base;
    cutoff /= base;
    for(;; c = *s++)
    {
    ...
    if(any < 0)
    {
        acc          = neg ? (-(0x7fffffffffffffffL) - 1) : (0x7fffffffffffffffL);
        rptr->_errno = 34;
    }
    else if(!any)
    {
    noconv:
        ...
        acc = -acc;
    if(endptr != ((void*)0)) *endptr = (char*)(any ? s - 1 : nptr);
    return (acc);
}
intmax_t _strtoimax_r(struct _reent* rptr, const char* restrict nptr, char** restrict endptr, int base)
{
    return _strtoimax_l(rptr, nptr, endptr, base, __get_current_locale());
}
intmax_t strtoimax_l(const char* restrict nptr, char** restrict endptr, int base, locale_t loc)
{
    return _strtoimax_l(_impure_ptr, nptr, endptr, base, loc);
}
intmax_t strtoimax(const char* restrict nptr, char** restrict endptr, int base)
{
    return _strtoimax_l(_impure_ptr, nptr, endptr, base, __get_current_locale());
}
