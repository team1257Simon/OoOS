...
_Bigint*                   _Balloc(struct _reent* ptr, int k)
{
    ...
    {
        ((ptr)->_freelist) = (struct _Bigint**)_calloc_r(ptr, sizeof(struct _Bigint*), (sizeof(size_t) << 3) + 1);
        if(((ptr)->_freelist) == ((void*)0)) { return ((void*)0); }
    }
    if((rv = ((ptr)->_freelist)[k]) != 0) { ((ptr)->_freelist)[k] = rv->_next; }
    else
    {
        x  = 1 << k;
...
_Bigint* __s2b(struct _reent* ptr, const char* s, int nd0, int nd, __ULong y9)
{
    _Bigint* b;
    int      i, k;
    int      x, y;
    ...
    {
        s += 9;
        do b = __multadd(ptr, b, 10, *s++ - '0');
        while(++i < nd0);
        s++;
    }
    else
        s += 10;
    for(; i < nd; i++) b = __multadd(ptr, b, 10, *s++ - '0');
    return b;
}
int __hi0bits(register __ULong x)
{
    register int k = 0;
    if(!(x & 0xffff0000))
    {
...
_Bigint* __pow5mult(struct _reent* ptr, _Bigint* b, int k)
{
    ...
    if((i = k & 3) != 0) b = __multadd(ptr, b, p05[i - 1], 0);
    if(!(k >>= 2)) return b;
    ;
    if(!(p5 = ((ptr)->_p5s)))
    {
...
_Bigint* __d2b(struct _reent* ptr, double _d, int* e, int* bits)
{
    ...
    (d.i[1]) &= 0x7fffffff;
    if((de = (int)((d.i[1]) >> 20)) != 0) z |= ((__uint32_t)0x100000L);
    if((d.i[0]))
    {
        y = (d.i[0]);
        k = __lo0bits(&y);
...
const double __mprec_tens[]     = {1e0,  1e1,  1e2,  1e3,  1e4,  1e5,  1e6,  1e7,  1e8,  1e9,  1e10, 1e11, 1e12,
...
{
    double v = 1.0;
    if(dig < 24) return __mprec_tens[dig];
    while(dig > 0)
    {
        v *= 10;
        dig--;
