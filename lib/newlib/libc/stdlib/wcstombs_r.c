...
size_t _wcstombs_r(struct _reent* r, char* restrict s, const wchar_t* restrict pwcs, size_t n, mbstate_t* state)
{
    char*  ptr = s;
    size_t max = n;
    char   buff[8];
    ...
    if(s == ((void*)0))
    {
        ...
        while(*pwcs != 0)
        {
            bytes = (__get_current_locale()->wctomb)(r, buff, *pwcs++, state);
            if(bytes == -1) return -1;
            num_bytes += bytes;
        }
        return num_bytes;
