...
sum (struct _reent *p, _Bigint *a, _Bigint *b)
{
 ...
 if (carry) {
  if (c->_wds == c->_maxwds) {
   b = _Balloc(p, c->_k + 1);
   memcpy((char *)&b->_sign, (char *)&c->_sign, c->_wds*sizeof(
  int
  ) + 2*sizeof(int));
   _Bfree(p, c);
   c = b;
      ...
      int rd, int *irv)
{
 ...
 if (e < fpi->emin) {
   ...
   *irv = STRTOG_Underflow | STRTOG_Inexlo;
   ...
   k1 = k - 1;
   if (k1 > 0 && !lostbits)
    lostbits = __any_on(b, k1);
   if (!lostbits && !exact)
    goto ret;
   lostbits |=
     carry = b->_x[k1>>5] & (1 << (k1 & 31));
   rshift(b, k);
   *irv = STRTOG_Denormal;
   if (carry) {
    b = increment(p, b);
     ...
     __ULong *bits, locale_t loc)
{
 ...
 if (nbits <= 53 && nd <= 15
  ...
  if (!e) {
   if (rvOK(p, (rv.d), fpi, exp, bits, 1, rd, &irv))
    goto ret;
   }
  else if (e > 0) {
   if (e <= 22) {
    ...
    i = fivesbits[e] + mantbits(rv) <= 53;
               (rv.d) *= __mprec_tens[e];
    if (rvOK(p, (rv.d), fpi, exp, bits, i, rd, &irv))
     goto ret;
    e1 -= e;
    goto rv_notOK;
   ...
   if (e <= 22 + i) {
    ...
    if (rvOK(p, (rv.d), fpi, exp, bits, 0, rd, &irv))
     goto ret;
    e1 -= e2;
    }
   }
 ...
 if (e1 > 0) {
  ...
  if (e1 &= ~15) {
   e1 >>= 4;
   while(e1 >= (1 << (5 -1))) {
    e2 += (((rv.i[1]) & ((__uint32_t)0x7ff00000L))
     >> 20) - 1023;
 ...
 int
  ...
  bb = _Balloc(p,rvb->_k);
  memcpy((char *)&bb->_sign, (char *)&rvb->_sign, rvb->_wds*sizeof(
 int
 ) + 2*sizeof(int));
  bbbits = rvbits - bb0;
  bbe = rve + bb0;
  ...
  if (bb2 > 0)
   ...
   rshift(bb, -bb2);
  if (bd5 > 0)
   bd = __pow5mult(p, bd, bd5);
  if (bd2 > 0)
   bd = __lshift(p, bd, bd2);
  if (bs2 > 0)
   bs = __lshift(p, bs, bs2);
  asub = 1;
  inex = STRTOG_Inexhi;
  ...
  if (rd && i <= 0) {
   ...
   if ( (finished = dsign ^ (rd&1)) !=0) {
    ...
    if (rve1 == emin)
     goto adj1;
    for(i = 0, j = nbits; j >= 32;
      i++, j -= 32) {
     if (rvb->_x[i] & 0xffffffff)
      goto adj1;
     }
    if (j > 1 && __lo0bits(rvb->_x + i) < j - 1)
     goto adj1;
    rve = rve1 - 1;
 ...
 drop_down:
   ...
   if (dsign) {
    rvb = increment(p, rvb);
    j = 31 & (32 - (rvbits & 31));
    if (__hi0bits(rvb->_x[rvb->_wds - 1]) != j)
     rvbits++;
    irv = STRTOG_Normal | STRTOG_Inexhi;
    }
   else {
 ...
 inc_L:
  ...
  if (asub) {
   ...
   k = rvb->_wds - 1;
   if (k >= rvb0->_wds
    || __hi0bits(rvb->_x[k]) < __hi0bits(rvb0->_x[k])) {
    if (denorm) {
     if (++rvbits == nbits)
      denorm = 0;
     }
