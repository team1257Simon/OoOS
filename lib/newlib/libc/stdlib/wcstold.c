...
long double                                   wcstold_l(const wchar_t* restrict nptr, wchar_t** restrict endptr, locale_t loc)
{
    ...
    wcp = nptr;
    mbs = initial;
    if((len = _wcsnrtombs_l(_impure_ptr, ((void*)0), &wcp, (size_t)-1, 0, &mbs, loc)) == (size_t)-1)
    {
        if(endptr != ((void*)0)) *endptr = (wchar_t*)nptr;
        return 0.0L;
    }
    if((buf = malloc(len + 1)) == ((void*)0)) return 0.0L;
    mbs = initial;
    _wcsnrtombs_l(_impure_ptr, buf, &wcp, (size_t)-1, len + 1, &mbs, loc);
...
long double wcstold(const wchar_t* restrict nptr, wchar_t** restrict endptr) { return wcstold_l(nptr, endptr, __get_current_locale()); }
