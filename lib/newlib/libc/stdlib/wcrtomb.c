...
size_t                                        _wcrtomb_r(struct _reent* ptr, char* s, wchar_t wc, mbstate_t* ps)
{
    ...
    char buf[10];
    if(s == ((void*)0))
        retval = (__get_current_locale()->wctomb)(ptr, buf, L'\0', ps);
    else
        retval = (__get_current_locale()->wctomb)(ptr, s, wc, ps);
    if(retval == -1)
    {
        ps->__count = 0;
        ptr->_errno = 138;
...
size_t wcrtomb(char* restrict s, wchar_t wc, mbstate_t* restrict ps)
{
    ...
    char           buf[10];
    if(s == ((void*)0))
        retval = (__get_current_locale()->wctomb)(reent, buf, L'\0', ps);
    else
        retval = (__get_current_locale()->wctomb)(reent, s, wc, ps);
    if(retval == -1)
    {
        ps->__count   = 0;
        reent->_errno = 138;
