...
static void                print_f(struct _reent* ptr, char* buf, double invalue, int ndigit, char type, int dot, int mode)
{
    int   decpt;
    int   sign;
    char *p, *start, *end;
...
char* fcvtbuf(double invalue, int ndigit, int* decpt, int* sign, char* fcvt_buf)
{
    ...
    if(fcvt_buf == ((void*)0))
    {
        ...
        {
            if((fcvt_buf = (char*)_realloc_r(reent, reent->_cvtbuf, ndigit + 36)) == ((void*)0)) return ((void*)0);
    ...
    save = fcvt_buf;
    if(invalue < 1.0 && invalue > -1.0) { p = _dtoa_r(reent, invalue, 2, ndigit, decpt, sign, &end); }
    else { p = _dtoa_r(reent, invalue, 3, ndigit, decpt, sign, &end); }
    done = -*decpt;
    while(p < end)
    {
        *fcvt_buf++ = *p++;
...
char* ecvtbuf(double invalue, int ndigit, int* decpt, int* sign, char* fcvt_buf)
{
    ...
    if(fcvt_buf == ((void*)0))
    {
        ...
        {
            if((fcvt_buf = (char*)_realloc_r(reent, reent->_cvtbuf, ndigit + 1)) == ((void*)0)) return ((void*)0);
...
{
    char* save = buf;
    if(invalue < 0) { invalue = -invalue; }
    if(invalue == 0)
    {
        *buf++ = '0';
        *buf   = '\0';
    }
    else if(0.0001 >= invalue || invalue >= _mprec_log10(ndigit)) { print_e(ptr, buf, invalue, ndigit - 1, type, dot); }
    else
    {
        int   decpt;
        int   sign;
        char* end;
        char* p;
        if(invalue < 1.0) { p = _dtoa_r(ptr, invalue, 3, ndigit, &decpt, &sign, &end); }
        else { p = _dtoa_r(ptr, invalue, 2, ndigit, &decpt, &sign, &end); }
        if(decpt == 9999)
        {
            strcpy(buf, p);
            return save;
...
char* _dcvt(struct _reent* ptr, char* buffer, double invalue, int precision, int width, char type, int dot)
{
    switch(type)
    {
    case 'f':
    case 'F': print_f(ptr, buffer, invalue, precision, type, precision == 0 ? dot : 1, 3); break;
    case 'g':
    case 'G':
