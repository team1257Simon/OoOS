...
extern long double strtold_l(const char* restrict, char** restrict, locale_t);
long               strtol_l(const char* restrict, char** restrict, int, locale_t);
unsigned long      strtoul_l(const char* restrict, char** restrict, int, locale_t __loc);
long long          strtoll_l(const char* restrict, char** restrict, int, locale_t);
unsigned long long strtoull_l(const char* restrict, char** restrict, int, locale_t __loc);
...
char* __attribute__((__nonnull__(1))) basename(const char*) __asm__(""
                                                                    "__gnu_basename");
typedef union
{
    double value;
...
long long          wcstoll_l(const wchar_t* restrict, wchar_t** restrict, int, locale_t);
unsigned long      wcstoul_l(const wchar_t* restrict, wchar_t** restrict, int, locale_t);
unsigned long long wcstoull_l(const wchar_t* restrict, wchar_t** restrict, int, locale_t);
...
static double          sulp(U x, int scale)
{
    ...
    int    i;
    rv = __ulp((x.d));
    if(!scale || (i = 2 * 53 + 1 - (((x.i[1]) & ((__uint32_t)0x7ff00000L)) >> 20)) <= 0) return rv;
...
static void ULtod(__ULong* L, __ULong* bits, Long exp, int k)
{
    ...
    case STRTOG_Infinite:
        ...
        L[0] = 0;
        break;
    case STRTOG_NaN: L[1] = 0x7fffffff; L[0] = (__ULong)-1;
    }
    if(k & STRTOG_Neg) L[1] |= 0x80000000L;
}
double _strtod_l(struct _reent* ptr, const char* restrict s00, char** restrict se, locale_t loc)
{
    int         scale;
    ...
    Long        L;
    __ULong     y, z;
    _Bigint *   bb = ((void*)0), *bb1, *bd = ((void*)0), *bd0, *bs = ((void*)0), *delta = ((void*)0);
    __ULong     Lsb, Lsb1;
    ...
    {
        {
            ...
            case 'X':
                s00 = s + 1;
                {
                    ...
                    {
                    case STRTOG_NoNumber: s = s00; sign = 0;
...
dig_done:
    ...
    if(!nd)
    {
        ...
        {
            __ULong          bits[2];
            static const FPI fpinan = {52, 1 - 1023 - 53 + 1, 2046 - 1023 - 53 + 1, 1, 0};
            if(!decpt) switch(c)
                {
                case 'i':
                case 'I':
                ...
                case 'N':
                    if(__match(&s, "an"))
                    {
                        if(*s == '(' && __hexnan(&s, &fpinan, bits) == STRTOG_NaNbits)
                        {
    ...
    if(e1 > 0)
    {
        ...
        if(e1 &= ~15)
        {
            ...
            if((z = (rv.i[1]) & ((__uint32_t)0x7ff00000L)) > ((__uint32_t)0x100000L) * (1024 + 1023 - 53)) goto ovfl;
            if(z > ((__uint32_t)0x100000L) * (1024 + 1023 - 1 - 53))
            {
                (rv.i[1]) = (((__uint32_t)0xfffffL) | ((__uint32_t)0x100000L) * (1024 + 1023 - 1));
    ...
    else if(e1 < 0)
    {
        ...
        if(e1 >>= 4)
        {
            if(e1 >= 1 << 5) goto undfl;
            if(e1 & 0x10) scale = 2 * 53;
            for(j = 0; e1 > 0; j++, e1 >>= 1)
                if(e1 & 1) (rv.d) *= tinytens[j];
            if(scale && (j = 2 * 53 + 1 - (((rv.i[1]) & ((__uint32_t)0x7ff00000L)) >> 20)) > 0)
            {
                if(j >= 32)
                {
                    (rv.i[0]) = 0;
    ...
    for(;;)
    {
        ...
        if(i < 0)
        {
            ...
            if(!delta->_x[0] && delta->_wds <= 1) { break; }
            delta = __lshift(ptr, delta, 1);
            if(__mcmp(delta, bs) > 0) goto drop_down;
            break;
        }
        if(i == 0)
        {
            if(dsign)
            {
                if(((rv.i[1]) & ((__uint32_t)0xfffffL)) == ((__uint32_t)0xfffffL) &&
                   (rv.i[0]) == ((scale && (y = (rv.i[1]) & ((__uint32_t)0x7ff00000L)) <= 2 * 53 * ((__uint32_t)0x100000L)) ?
                                     (0xffffffff & (0xffffffff << (2 * 53 + 1 - (y >> 20)))) :
                                     0xffffffff))
                {
                    if((rv.i[1]) == (((__uint32_t)0xfffffL) | ((__uint32_t)0x100000L) * (1024 + 1023 - 1)) && (rv.i[0]) == ((__uint32_t)0xffffffffL))
                        goto ovfl;
                    (rv.i[1]) = ((rv.i[1]) & ((__uint32_t)0x7ff00000L)) + ((__uint32_t)0x100000L);
                    (rv.i[0]) = 0;
                    dsign     = 0;
                    break;
                }
            ...
            drop_down:
                if(scale)
                {
                    ...
                    {
                        if(L > (53 + 2) * ((__uint32_t)0x100000L)) break;
                        goto undfl;
                    }
                }
        ...
        if(y == ((__uint32_t)0x100000L) * (1024 + 1023 - 1))
        {
            ...
            (rv.d) += adj;
            if(((rv.i[1]) & ((__uint32_t)0x7ff00000L)) >= ((__uint32_t)0x100000L) * (1024 + 1023 - 53))
            {
                if((rv0.i[1]) == (((__uint32_t)0xfffffL) | ((__uint32_t)0x100000L) * (1024 + 1023 - 1)) && (rv0.i[0]) == ((__uint32_t)0xffffffffL))
                    goto ovfl;
                (rv.i[1]) = (((__uint32_t)0xfffffL) | ((__uint32_t)0x100000L) * (1024 + 1023 - 1));
                (rv.i[0]) = ((__uint32_t)0xffffffffL);
                goto cont;
    ...
    if(scale)
    {
        ...
        (rv0.i[0]) = 0;
        (rv.d) *= (rv0.d);
        if((rv.i[1]) == 0 && (rv.i[0]) == 0) ptr->_errno = 34;
    }
retfree:
    _Bfree(ptr, bb);
    _Bfree(ptr, bd);
...
double _strtod_r(struct _reent* ptr, const char* restrict s00, char** restrict se) { return _strtod_l(ptr, s00, se, __get_current_locale()); }
double strtod_l(const char* restrict s00, char** restrict se, locale_t loc) { return _strtod_l(_impure_ptr, s00, se, loc); }
double strtod(const char* restrict s00, char** restrict se) { return _strtod_l(_impure_ptr, s00, se, __get_current_locale()); }
float  strtof_l(const char* restrict s00, char** restrict se, locale_t loc)
{
    double val = _strtod_l(_impure_ptr, s00, se, loc);
    if((__builtin_isnan(val)))
        return ((sizeof(val) == sizeof(float))  ? __builtin_signbitf(val) :
                (sizeof(val) == sizeof(double)) ? __builtin_signbit(val) :
...
{
    double val = _strtod_l(_impure_ptr, s00, se, __get_current_locale());
    if((__builtin_isnan(val)))
        return ((sizeof(val) == sizeof(float))  ? __builtin_signbitf(val) :
                (sizeof(val) == sizeof(double)) ? __builtin_signbit(val) :
                                                  __builtin_signbitl(val)) ?
