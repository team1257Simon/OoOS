...
static unsigned long long _strtoull_l(struct _reent* rptr, const char* restrict nptr, char** restrict endptr, int base, locale_t loc)
{
    ...
    do {
        c = *s++;
    } while((((_ctype_ + sizeof(""[c]))[(int)(c)]) & 010));
    if(c == '-')
    {
        neg = 1;
        c   = *s++;
    ...
    cutoff = (unsigned long long)(0x7fffffffffffffffLL * 2ULL + 1ULL) / (unsigned long long)base;
    cutlim = (unsigned long long)(0x7fffffffffffffffLL * 2ULL + 1ULL) % (unsigned long long)base;
    for(acc = 0, any = 0;; c = *s++)
    {
        if(c >= '0' && c <= '9')
            c -= '0';
    ...
    if(any < 0)
    {
        acc          = (0x7fffffffffffffffLL * 2ULL + 1ULL);
        rptr->_errno = 34;
    }
    else if(neg)
        acc = -acc;
...
unsigned long long _strtoull_r(struct _reent* rptr, const char* restrict nptr, char** restrict endptr, int base)
{
    return _strtoull_l(rptr, nptr, endptr, base, __get_current_locale());
}
unsigned long long strtoull_l(const char* restrict s, char** restrict ptr, int base, locale_t loc)
{
    return _strtoull_l(_impure_ptr, s, ptr, base, loc);
}
unsigned long long strtoull(const char* restrict s, char** restrict ptr, int base)
{
    return _strtoull_l(_impure_ptr, s, ptr, base, __get_current_locale());
}
