...
void                         _malloc_stats_r(struct _reent* reent_ptr)
{
    ...
    local_max_total_mem = __malloc_max_total_mem;
    local_sbrked_mem    = (__malloc_current_mallinfo.arena);
    local_mallinfo      = __malloc_current_mallinfo;
    __malloc_unlock(reent_ptr);
    ;
    fp = ((reent_ptr)->_stderr);
    fiprintf(fp, "max system bytes = %10u\n", (unsigned int)(local_max_total_mem));
    fiprintf(fp, "system bytes     = %10u\n", (unsigned int)local_sbrked_mem);
    fiprintf(fp, "in use bytes     = %10u\n", (unsigned int)local_mallinfo.uordblks);
}
