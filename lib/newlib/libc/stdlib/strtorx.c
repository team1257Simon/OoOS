...
void                   ULtox(__UShort* L, __ULong* bits, Long exp, int k)
{
    switch(k & STRTOG_Retmask)
    {
    case STRTOG_NoNumber:
    case STRTOG_Zero: L[0] = L[1] = L[2] = L[3] = L[4] = 0; break;
    case STRTOG_Denormal: L[4] = 0; goto normal_bits;
    case STRTOG_Normal:
    case STRTOG_NaNbits:
        L[4] = exp + 0x3fff + 63;
    normal_bits:
        L[0] = (__UShort)bits[0];
        L[1] = (__UShort)(bits[0] >> 16);
        L[2] = (__UShort)bits[1];
        L[3] = (__UShort)(bits[1] >> 16);
        break;
    case STRTOG_Infinite:
        ...
        L[2] = L[1] = L[0] = 0;
        break;
    case STRTOG_NaN: *((long double*)L) = __builtin_nanl("");
    }
    if(k & STRTOG_Neg) L[4] |= 0x8000;
}
int _strtorx_l(struct _reent* p, const char* s, char** sp, int rounding, void* L, locale_t loc)
{
