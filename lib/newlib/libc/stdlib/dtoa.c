...
char* _dtoa_r(struct _reent* ptr, double _d, int mode, int ndigits, int* decpt, int* sign, char** rve)
{
    ...
    else
    {
        ...
        x    = (i > 32) ? ((d.i[1]) << (64 - i)) | ((d.i[0]) >> (i - 32)) : ((d.i[0]) << (32 - i));
        d2.d = x;
    ...
    if(leftright)
    {
        if(mode < 2) { i = denorm ? be + (1023 + (53 - 1) - 1 + 1) : 1 + 53 - bbits; }
        else
        {
            j = ilim - 1;
            if(m5 >= j)
    ...
    if(b5 > 0)
    {
        if(leftright)
        {
            ...
            if((j = b5 - m5) != 0) b = __pow5mult(ptr, b, j);
        }
        else
            b = __pow5mult(ptr, b, b5);
    }
    S = __i2b(ptr, 1);
    if(s5 > 0) S = __pow5mult(ptr, S, s5);
    spec_case = 0;
    if(mode < 2)
    {
        if(!(d.i[0]) && !((d.i[1]) & ((__uint32_t)0xfffffL)) && (d.i[1]) & ((__uint32_t)0x7ff00000L))
        {
            b2 += 1;
            s2 += 1;
            spec_case = 1;
    ...
    if((i = ((s5 ? 32 - __hi0bits(S->_x[S->_wds - 1]) : 1) + s2) & 0x1f) != 0) i = 32 - i;
    if(i > 4)
    {
        i -= 4;
        b2 += i;
    ...
    if(b2 > 0) b = __lshift(ptr, b, b2);
    if(s2 > 0) S = __lshift(ptr, S, s2);
    if(k_check)
    {
        if(__mcmp(b, S) < 0)
        {
            ...
            if(leftright) mhi = __multadd(ptr, mhi, 10, 0);
            ilim = ilim1;
        }
    }
    if(ilim <= 0 && mode > 2)
    ...
    if(leftright)
    {
        if(m2 > 0) mhi = __lshift(ptr, mhi, m2);
        mlo = mhi;
        if(spec_case)
        {
            mhi = _Balloc(ptr, mhi->_k);
            memcpy((char*)&mhi->_sign, (char*)&mlo->_sign, mlo->_wds * sizeof(int) + 2 * sizeof(int));
