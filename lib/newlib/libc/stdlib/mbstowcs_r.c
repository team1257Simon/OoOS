...
size_t _mbstowcs_r(struct _reent* r, wchar_t* restrict pwcs, const char* restrict s, size_t n, mbstate_t* state)
{
    size_t ret = 0;
    char*  t   = (char*)s;
    int    bytes;
    ...
    while(n > 0)
    {
        bytes = (__get_current_locale()->mbtowc)(r, pwcs, t, __locale_mb_cur_max(), state);
        if(bytes < 0)
        {
            state->__count = 0;
            return -1;
