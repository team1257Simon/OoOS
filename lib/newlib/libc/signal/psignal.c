       ...
       const char *s)
{
  ...
  if (s != 
          ((void *)0) 
               && *s != '\0')
    {
      { const char *p = (s); size_t len = strlen (p); while (len) { ssize_t len1 = write (fileno (
     (_impure_ptr->_stderr)
     ), p, len); if (len1 < 0) break; len -= len1; p += len1; } };
      { const char *p = (": "); size_t len = strlen (p); while (len) { ssize_t len1 = write (fileno (
     (_impure_ptr->_stderr)
     ), p, len); if (len1 < 0) break; len -= len1; p += len1; } };
    }
  { const char *p = (strsignal (sig)); size_t len = strlen (p); while (len) { ssize_t len1 = write (fileno (
 (_impure_ptr->_stderr)
 ), p, len); if (len1 < 0) break; len -= len1; p += len1; } };
  ...
  { const char *p = ("\n"); size_t len = strlen (p); while (len) { ssize_t len1 = write (fileno (
 (_impure_ptr->_stderr)
 ), p, len); if (len1 < 0) break; len -= len1; p += len1; } };
