...
To build the OS, you will need to install the libc and targeted toolchain as well; I created a script(currently untested) to build that toolchain, but the procedure goes like this:
 ...
 14. Delete the entire contents of the newlib build directory, then configure again with `../newlib/configure --target=x86_64-ooos --prefix=$SYSROOT/usr --with-sysroot=$SYSROOT --enable-shared --enable-host-shared` to turn on the shared object build. You'll need to rebuild and reinstall newlib in order to build the images.
 15. The module builds require slightly different `crtbeginS.o` and `crtendS.o` object files instead of the standard `crtbegin.o` and `crtend.o` files GCC will build for the targeted cross-compiler. You should be able to find those two files in the library directory for any Linux GCC, and they work fine with the build. The weak symbols for transactional memory they reference will show up in the module files' dynsym tables, but as they're never used by module code it's safe to ignore them. If the modules ever do need transactional memory, I'll have to implement those things in the module object library, but for the moment that's not needed.
