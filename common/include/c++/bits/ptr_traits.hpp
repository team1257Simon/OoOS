...
namespace std
{
    ...
    template<typename PT, typename... None>
    constexpr auto __to_address(const PT& __ptr, None...) noexcept
    {
        if constexpr (is_base_of_v<__debug::__safe_iterator_base, PT>) return std::__to_address(__ptr.base().operator->());
        else return std::__to_address(__ptr.operator->());
    }
