...
namespace std::__impl
{
    ...
    struct __dynamic_queue
    {
        ...
        constexpr void __qset(__ptr where, __value_type const& val, __size_type n) { array_fill(where, val, n); }
        ...
        constexpr void __bumpn(__diff_type n = __diff_type(1L)) noexcept { __my_queue_data.__bumpn(n); }
        constexpr void __bumpe(__diff_type n = __diff_type(1L)) noexcept { __my_queue_data.__bumpc(n); }
        ...
        constexpr __ptr __unpop() noexcept { if(__qcur() > __qbeg()) { __bumpn(-1); __op_cnt = 0; return __qcur(); } return nullptr; }
        ...
        constexpr void __qmove(__dynamic_queue&& that) noexcept { __my_queue_data.__move(move(that.__my_queue_data)); }
        ...
        constexpr __dynamic_queue& operator=(__dynamic_queue&& that) { __qdestroy(); __qmove(move(that)); return *this; }
        constexpr __dynamic_queue& operator=(__dynamic_queue const& that) { __qdestroy(); __qpcopy(that); return *this; }
    };
    template <typename T, allocator_object<T> A>
    constexpr typename __dynamic_queue<T, A>::__size_type __dynamic_queue<T, A>::__erase_before_next()
    {
        ...
        __qcopy(tmp.__begin, __qcur(), __qrem());
        tmp.__bumpc(__qrem());
        __qdestroy();
    ...
    constexpr bool __dynamic_queue<T, A>::__q_grow_buffer(typename __dynamic_queue<T, A>::__size_type added)
    {
        ...
        try { __my_queue_data.__set_ptrs(resize(__qbeg(), __qcapacity(), target_cap), nx_offs, num_elements, target_cap); }
        catch(...) { return false; }
        return true;
    }
    template <typename T, allocator_object<T> A>
