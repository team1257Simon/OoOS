...
namespace std
{
    ...
    template<> struct hash<void*> { uint64_t operator()(void* const& ptr) const noexcept { return elf64_hash()(&ptr, sizeof(void*)); } };
    template<> struct hash<addr_t> { uint64_t operator()(addr_t const& ptr) const noexcept { return elf64_hash()(&ptr, sizeof(addr_t)); } };
}
#endif