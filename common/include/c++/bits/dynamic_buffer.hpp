...
namespace std::__impl
{
    ...
    protected:
        ...
        void __append_elements(size_t count, T const& t)
        {
            if(__my_data.__max < __my_data.__end + count) __grow_buffer(size_t(count - __rem()));
        ...
        __dynamic_buffer(IT start, IT end, A const& alloc) : __allocator{ alloc }, __my_data{ __allocator.allocate(size_t(end - start)), size_t(end - start) } { size_t n = end - start; __copy(__my_data.__begin, start, n); __advance(n); }
