...
namespace std
{
    ...
    template<typename T> constexpr void fill(T* first, T* last, T const& value) { array_fill<T>(first, value, distance(first, last)); }
    template<typename T> constexpr void fill_n(T* first, size_t n, T const& value) { array_fill<T>(first, value, n); }
