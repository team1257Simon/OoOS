...
namespace std
{
	...
	template<typename T> constexpr void fill_n(T* first, size_t n, T const& value) { if constexpr(trivial_copy<T>) array_fill<T>(first, value, n); else if constexpr(copy_constructible<T>) array_fill<T>(first, tuple<T const&>(value), n); }
