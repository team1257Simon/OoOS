...
namespace std
{
    ...
    namespace __impl
    {
        ...
        struct __hashtable : __hashtable_base
        {
            ...
            constexpr size_type __hash_code(__const_node_ptr n) const { return __hash_fn()(__extr_fn()(n->__ref())); }
            constexpr size_type __hash_code(key_type const& k) const { return __hash_fn()(k); }
            constexpr size_type __hash_code(value_type const& v) const { return __hash_fn()(__extr_fn()(v)); }
    ...
    template<typename KT, typename MT, __detail::__hash_ftor<KT> HT, __detail::__predicate<KT> ET = equal_to<void>, allocator_object<__impl::__hash_node<pair<const KT, MT>>> AT = std::allocator<__impl::__hash_node<pair<const KT, MT>>>>
    ...
    private:
        constexpr pair<iterator, bool> __insert_or_assign(__node_ptr n) { __node_ptr p = this->__find(n->__ref().first); if(p) { p->__ref().second = move(n->__ref().second); this->__destroy_node(n); return make_pair(iterator(p), false); } return make_pair(this->__insert(n), true); }
    public:
        ...
        constexpr iterator begin() { return iterator(this->__begin()); }
        constexpr const_iterator cbegin() const { return const_iterator(this->__begin()); }
    ...
    template<typename T, __detail::__hash_ftor<T> HT, __detail::__predicate<T> ET = equal_to<void>, allocator_object<__impl::__hash_node<T>> AT = std::allocator<__impl::__hash_node<T>>>
