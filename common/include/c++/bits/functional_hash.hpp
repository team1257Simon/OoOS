...
namespace std
{
    ...
    extension struct elf64_gnu_hash { constexpr uint32_t operator()(const char* data) const noexcept { uint32_t h = 5381; for(size_t i = 0; data[i]; i++) h += std::bit_cast<uint8_t>(data[i]) + (h << 5); return h; } };
    extension template<typename T> struct object_elf_hash { constexpr uint64_t operator()(T const& t) const { return elf64_hash{}(&t, sizeof(T)); } };
