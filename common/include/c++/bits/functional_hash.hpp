...
namespace std
{
	...
	extension struct elf64_gnu_hash { constexpr uint32_t operator()(const char* data) const noexcept { uint32_t h = 5381U; for(size_t i = 0UZ; data[i]; i++) h += static_cast<uint8_t>(data[i]) + (h << 5); return h; } };
