...
namespace std
{
	...
	public:
		...
		constexpr pair<iterator, bool> emplace(Args&& ... args) { return this->__emplace(forward<Args>(args)...); }
		template<convertible_to<key_type> JT, convertible_to<mapped_type> NT>
		constexpr pair<iterator, bool> insert(pair<JT, NT>&& p) { return this->__emplace(piecewise_construct, forward_as_tuple(move(p.first)), forward_as_tuple(move(p.second))); }
		template<convertible_to<key_type> JT, convertible_to<mapped_type> NT>
		constexpr pair<iterator, bool> insert(pair<JT, NT> const& p) { return this->__emplace(piecewise_construct, tuple<JT const&>(p.first), tuple<NT const&>(p.second)); }
		template<std::convertible_to<key_type> JT> requires(is_default_constructible_v<mapped_type>)
		constexpr mapped_type& operator[](JT&& key) { return this->__emplace(piecewise_construct, forward_as_tuple(move(key)), tuple<>()).first->second; }
		template<std::convertible_to<key_type> JT> requires(is_default_constructible_v<mapped_type>)
		constexpr mapped_type& operator[](JT const& key) { return this->__emplace(piecewise_construct, tuple<JT const&>(key), tuple<>()).first->second; }
		template<std::input_iterator IT> requires(constructible_from<value_type, deref_t<IT>>)
		constexpr void insert(IT start, IT end) { this->__insert(start, end); }
