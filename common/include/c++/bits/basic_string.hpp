...
namespace std
{
	...
	protected:
		...
		constexpr static size_type __ldist(IT start, IT end) { return end > start ? static_cast<size_type>(std::distance(start, end)) : 0UZ; }
	public:
		...
		constexpr basic_string(size_type count, value_type const& value, allocator_type const& alloc = allocator_type{}) : __base(count + 1UZ, alloc) {
			this->__set(this->__beg(), value, count);
			this->__setc(count);
		}
		template<matching_input_iterator<value_type> IT>
		constexpr basic_string(IT start, IT end, allocator_type const& alloc = allocator_type{}) : __base(__ldist(start, end) + 1UZ, alloc) {
			this->__transfer(data(), start, end);
			this->__setc(__ldist(start, end));
		}
		constexpr basic_string(const_pointer str, size_type count, allocator_type const& alloc = allocator_type{}) : __base(count + 1UZ, alloc) {
		...
		constexpr basic_string(basic_string const& that, allocator_type const& alloc = allocator_type{}) : basic_string(that.data(), that.size(), alloc) {}
		...
		constexpr basic_string(initializer_list<value_type> init, allocator_type const& alloc = allocator_type{}) : basic_string(init.begin(), init.end(), alloc) {}
	...
	template<char_type CT, char_traits_type<CT> TT, allocator_object<CT> AT> constexpr basic_string<CT, TT, AT> operator+(basic_string<CT, TT, AT> const& __this, basic_string<CT, TT, AT> const& __that) { basic_string<CT, TT, AT> result(__this, __this.get_allocator()); result.append(__that); return result; }
	...
	template<char_type CT, char_traits_type<CT> TT, allocator_object<CT> AT> constexpr basic_string<CT, TT, AT> operator+(basic_string<CT, TT, AT> const& __this, const CT* __that) { basic_string<CT, TT, AT> result(__this, __this.get_allocator()); result.append(__that); return result; }
	template<char_type CT, char_traits_type<CT> TT, allocator_object<CT> AT> constexpr basic_string<CT, TT, AT> operator+(basic_string<CT, TT, AT> const& __this, CT __that) { basic_string<CT, TT, AT> result(__this, __this.get_allocator()); result.append(__that); return result; }
