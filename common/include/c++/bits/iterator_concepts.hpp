...
namespace std
{
	...
	namespace __detail
	{
		template<typename T> struct __iter_value { typedef typename __iter_traits<T, indirectly_readable_traits<T>>::value_type type; };
		template<typename T> using __iter_value_t = typename __iter_value<T>::type;
	...
	namespace __detail { template<typename IT, typename PT> struct __iter_value<projected<IT, PT>> { typedef std::invoke_result_t<PT&, __iter_value_t<IT>&> type; }; }
	template<indirectly_readable IT, indirectly_regular_unary_invocable<IT> PT> requires(weakly_incrementable<IT>) struct projected<IT, PT> { using value_type = remove_cvref_t<indirect_result_t<PT&, IT>>; indirect_result_t<PT&, IT> operator*() const; using difference_type = iter_difference_t<IT>; };
