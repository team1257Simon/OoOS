...
namespace std
{
    extension namespace ext
    {
        ...
        {
            try { expand(this->__out_region.__capacity(), ios_base::out); } catch(...) { return traits_type::eof(); }
            *this->pptr() = traits_type::to_char_type(c);
            this->pbump(1);
            return traits_type::not_eof(e);
        }
        template<char_type CT, char_traits_type<CT> TT, allocator_object<CT> AT>
        ...
        {
            if(pointer p = this->gptr(); __builtin_expect(p && this->eback() < p--))
            {
