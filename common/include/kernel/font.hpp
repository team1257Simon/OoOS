...
class font_render 
{
	...
	uint32_t __fb_width;
	constexpr size_t __buffer_start_offset(point const& start) const { return start.x * __my_font->width + start.y * __fb_width * __my_font->height; }
	constexpr size_t __buffer_offset(point const& start, point const& sub) const { return __buffer_start_offset(start) + sub.y * __fb_width + sub.x; }
...
public:
	constexpr font_render(psf2_t* font_data, uint32_t fb_width, uint32_t fg_color, uint32_t bg_color) noexcept : 
		__my_font		{ font_data }, 
