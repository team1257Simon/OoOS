...
class font_render 
{
	...
	uint32_t __y_res;
	uint32_t __x_res;
	constexpr size_t __buffer_start_offset(point const& start) const { return (start.x * __my_font->width) * __x_res + (start.y * __my_font->height) * __y_res; }
	constexpr size_t __buffer_offset(point const& start, point const& sub) const { return ((__buffer_start_offset(start) + sub.y * __y_res) / __x_res) + sub.x; }
...
public:
	constexpr font_render(psf2_t* font_data, uint32_t fb_pitch, uint32_t fb_width, uint32_t fg_color, uint32_t bg_color) noexcept : 
		__my_font		{ font_data }, 
		__foreground	{ fg_color }, 
		__background	{ bg_color }, 
		__y_res			{ fb_pitch },
		__x_res			{ fb_pitch / fb_width }
						{}
	...
	constexpr void set_fb_pitch(uint32_t value) noexcept { __y_res = value; }
	...
	constexpr uint32_t fb_pitch() const noexcept { return __y_res; }
