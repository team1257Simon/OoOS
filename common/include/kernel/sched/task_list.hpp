...
class task_list : std::set<task_ctx>
{
    ...
    uint64_t __upid() const noexcept;
...
public:
    ...
    task_ctx* create_user_task(prog_desc_t const& program_desc, cstr_vec&& args, int64_t parent_pid = -1L, priority_val pv = priority_val::PVNORM, uint16_t quantum = 5, pid_t pid = 0);
