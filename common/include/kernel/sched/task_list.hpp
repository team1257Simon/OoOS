...
public:
    ...
    task_ctx* create_system_task(task_functor task, cstr_vec&& args, size_t stack_size, size_t tls_size, priority_val pv = priority_val::PVSYS, uint16_t quantum = 4);
    task_ctx* create_user_task(prog_desc_t const& program_desc, cstr_vec&& args, int64_t parent_pid = -1L, priority_val pv = priority_val::PVNORM, uint16_t quantum = 4, pid_t pid = 0);
