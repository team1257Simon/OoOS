...
public:
    ...
    task_ctx* create_user_task(task_functor task, std::vector<const char*>&& args, addr_t frame_ptr, addr_t stack_base, addr_t tls_base, int64_t parent_pid = -1L, size_t stack_size = S04, size_t tls_size = S04, priority_val pv = priority_val::PVNORM, uint16_t quantum = 3, pid_t pid = 0);
    task_ctx* create_user_task(elf64_program_descriptor const& program_desc, std::vector<const char*>&& args, int64_t parent_pid = -1L, priority_val pv = priority_val::PVNORM, uint16_t quantum = 3, pid_t pid = 0);
