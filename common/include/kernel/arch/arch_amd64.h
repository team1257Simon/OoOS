...
#ifdef __cplusplus
#include <concepts>
constexpr u8 sig_pic_eoi		=	0x20UC;		/* End-of-interrupt command code */
...
template<io_port_valid I = u8> constexpr I in(uint16_t from) { I result; asm volatile("in %1, %0" : "=a"(result) : "Nd"(from) : "memory"); return result; }
template<io_port_valid I = u8> constexpr void out(uint16_t to, I value) { asm volatile("out %0, %1" :: "a"(value), "Nd"(to) : "memory"); }
constexpr void outb(uint16_t to, u8 value) { out(to, value); }
constexpr u8 inb(uint16_t from) { return in(from); }
constexpr void io_wait() { outb(0x80US, 0); }
constexpr void outbw(uint16_t to, u8 value) { outb(to, value); io_wait(); }
constexpr u8 inbw(uint16_t from) { u8 result = inb(from); io_wait(); return result; }
constexpr void kb_wait() { for(uint8_t result = inb(command_keybd); result & 0x02UC; result = inb(command_keybd)); }
constexpr void kb_put(u8 b) { kb_wait(); outb(data_keybd, b); kb_wait(); }
constexpr u8 kb_get() { return inb(data_keybd); }
...
template<uint8_t R> constexpr void rtc_select() { u8 reg = R; u8 prev = inbw(command_rtc); outbw(command_rtc, (prev & 0x80) | R); }
template<uint8_t R> constexpr u8 read_rtc_register() { rtc_select<R>(); return inb(data_rtc); }
template<uint8_t R> constexpr void write_rtc_register(u8 val) { rtc_select<R>(); outb(data_rtc, val); }
constexpr bool is_cmos_update_in_progress() { return (read_rtc_register<0x0AUC>() & 0x80) != 0; }
constexpr u8 kb_ping() { kb_put(sig_keybd_ping); return kb_get(); }
