...
#include <sys/types.h>
/**
 * Finds the first string of zeroes in a bitmap of size num_ulongs * 64 whose length L fulfills (L >= num_zeroes && (L < 64 || L % 64 == 0)).
...
// Finds the position of the first zero in a bitmap of size num_ulongs * 64. The return value is the offset in total bits.
