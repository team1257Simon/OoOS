...
namespace ooos
{
	...
	private:
		...
		template<size_t S> requires(S < R - 1) constexpr multiarray<T, R - S> __index(scale_vector<S> const& pt) const noexcept { return multiarray<T, R - S>(__my_array + this->template __offset(pt), this->template __rem_dims<S>()); }
	...
	constexpr scale_vector<R> multiarray<T, R>::__compute_scales(scale_vector<R> const& dimensions) noexcept
	{
		...
		for(size_t i = 0UZ; i + 1 < R; i++)
			result[i] 		= dimensions.sub_volume(R - (i + 1));
		if constexpr(R > 0UZ)
			result[R - 1] 	= 1UZ;
		return result;
	}
